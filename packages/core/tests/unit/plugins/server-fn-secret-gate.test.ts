...
describe("Bug 44: server-fn-secret SSR gate", () => {
	it("should NOT resolve virtual:flare-server-fn-secret for client environment", () => {
		...
		const resolveId = sfnPlugin!.resolveId as (this: { environment?: { name?: string } }, id: string) => string | null;
		const clientResult = resolveId.call({ environment: { name: "client" } }, "virtual:flare-server-fn-secret");
	...
	it("should resolve virtual:flare-server-fn-secret for SSR environment", () => {
		...
		const resolveId = sfnPlugin!.resolveId as (this: { environment?: { name?: string } }, id: string) => string | null;
		const ssrResult = resolveId.call({ environment: { name: "ssr" } }, "virtual:flare-server-fn-secret");

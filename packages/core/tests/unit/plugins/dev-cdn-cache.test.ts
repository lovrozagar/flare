...
describe("parseSurrogateKey", () => {
	...
	it("multiple space-separated keys", () => {
		expect(parseSurrogateKey("page-about tag-blog tag-news")).toEqual([
			"page-about",
			"tag-blog",
			"tag-news",
		])
...
describe("buildCdnCacheKey", () => {
	...
	it("Vary: Accept-Language → appends dimension", () => {
		expect(
			buildCdnCacheKey("GET", "/about", ["accept-language"], { "accept-language": "en" }),
		).toBe("cdn:GET:/about:accept-language=en")
	})
	...
	it("missing request header value → empty string", () => {
		expect(buildCdnCacheKey("GET", "/about", ["accept-language"], {})).toBe(
			"cdn:GET:/about:accept-language=",
		)
	})
...
describe("isCacheable", () => {
	...
	it("Flare typical header with SWR → cacheable", () => {
		expect(
			isCacheable(200, parseCacheControl("public, s-maxage=3600, stale-while-revalidate=86400")),
		).toBe(true)
	})
})

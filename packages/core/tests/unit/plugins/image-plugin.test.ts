...
describe("createImagePlugin", () => {
	...
	describe("resolveId — edge cases", () => {
		...
		it("handles path with dots in directory names", () => {
			expect(plugin.resolveId?.("/assets/v2.0/hero.jpg")).toBe("\0flare-image:/assets/v2.0/hero.jpg");
		});
	...
	describe("load build mode (real sharp)", () => {
		const testImagePath = resolve(__dirname, "../../../../../e2e/apps/product/src/assets/test-hero.jpg");

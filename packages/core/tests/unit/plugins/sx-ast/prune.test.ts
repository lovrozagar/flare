...
function writeLibManifest(root: string, pkgName: string, classes: string[], subDir?: string): void {
...
describe("pruneFromLibManifests — CSS output", () => {
	it("omits lib class from consumer CSS when pruneFromLibManifests: true", () => {
		...
		callTransform(consumer, `export default function A() { return <div sx={{ color: "red" }} /> }`, "/src/a.tsx");
	...
	it("keeps consumer-only class when it is not in any lib manifest", () => {
		...
		callTransform(consumer, `export default function B() { return <div sx={{ margin: "16px" }} /> }`, "/src/b.tsx");
		const { css } = emitAll(consumer);

...
describe("rewriteModule — class= static resolution", () => {
	...
	it("explicit cn(...) → left as-is", () => {
		const src = `import { cn } from "@lovrozagar/flare/styles"; export default function A({ on }: { on: boolean }) { return <div class={cn("a", on && "b")} /> }`;
		const result = transform(src);
		if (!result) return;
...
describe("rewriteModule — existing flare/styles import merge (lines 478-491)", () => {
	it("merges compileCss into existing flare/styles import", () => {
		const src = `import { cn } from "@lovrozagar/flare/styles"
export default function A() { return <div css="color: red" /> }`;
		...
		const importMatches = result.code.match(/from "@lovrozagar\/flare\/styles"/g) ?? [];
	...
	it("all needed specifiers already present → import unchanged", () => {
		const src = `import { compileCss } from "@lovrozagar/flare/styles"
export default function A() { return <div css="color: red" /> }`;
		const result = transform(src);
		expect(result).not.toBeNull();
		if (!result) return;
	...
	it("empty import braces `import {} from` → inserts specifiers inside braces (line 490)", () => {
		 ...
		 */
		const src = `import {} from "@lovrozagar/flare/styles"
export default function A() { return <div css="color: red" /> }`;
		const result = transform(src);
		expect(result).not.toBeNull();
		if (!result) return;
	...
	it("no existing imports at all → prepends import (line 499 ms.prepend)", () => {
		...
		expect(result).not.toBeNull();
		if (!result) return;
		expect(result.code).toContain('from "@lovrozagar/flare/styles"');
		/* Import should appear before the function (no leading newline on prepend) */
		const importIdx = result.code.indexOf('@lovrozagar/flare/styles"');
...
export default function A() { return <div css="color: red" /> }`;
		...
		const stylesIdx = result.code.indexOf('@lovrozagar/flare/styles"');
...
describe("rewriteModule — emitClassAttr with expression literal=null (line 242)", () => {
	it("class={cn(...)} + sx static → emitClassAttr emits class={cn(..., atomics)} expression form", () => {
		 ...
		 */
		const src = `import { cn } from "@lovrozagar/flare/styles"; export default function A({ on }: { on: boolean }) { return <div class={cn("base", on && "active")} sx={{ color: "red" }} /> }`;
		const result = transform(src);
		expect(result).not.toBeNull();
		if (!result) return;
		expect(result.code).not.toContain("sx=");

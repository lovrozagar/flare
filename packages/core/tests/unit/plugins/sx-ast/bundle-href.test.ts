...
function runGenerateBundle(plugin: ReturnType<typeof createSxAstPlugin>): Record<string, string> {
	...
	const fn = plugin.generateBundle as (
		this: GenerateBundleCtx,
		opts: unknown,
		bundle: unknown,
	) => void
	fn.call({ emitFile }, {}, {})
...
describe("sx-ast bundleHref", () => {
	...
	it("sx-bundle-href-custom", () => {
		const plugin = (createSxAstPlugin as unknown as (opts: object, assetsBase: string) => ReturnType<typeof createSxAstPlugin>)(
			{ manifest: true },
			"/app/assets",
		)
		const emitted = runGenerateBundle(plugin)

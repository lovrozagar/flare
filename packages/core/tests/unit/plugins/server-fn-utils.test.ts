...
import { dropDeadImports, generateServerFnMapSource, stripHandlerBodies } from "../../../src/plugins/index.ts";
...
describe("stripHandlerBodies", () => {
	...
	it("handles multiple handlers in same file", () => {
		const code = [".handler(({ input }) => dbQuery(input))", '.handler(() => { throw new Error("boom") })'].join("\n");
		const result = stripHandlerBodies(code);
...
describe("stripHandlerBodies - stream", () => {
	...
	it("preserves code before and after .stream()", () => {
		const code = 'const x = createServerFn({ name: "a" }).stream(async function* () { yield secret }).someChain()';
		const result = stripHandlerBodies(code);
		expect(result).toContain("createServerFn");

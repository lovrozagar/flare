...
describe("buildPrerenderRoutes deep", () => {
	...
	it("throws on authenticate + isr", () => {
		expect(() =>
			buildPrerenderRoutes([
				makeDef({ authenticateMode: true, cache: { isr: true, isrRevalidate: 60 } }),
			] as never),
	...
	it("isr with revalidate → mode isr", () => {
		const routes = buildPrerenderRoutes([
			makeDef({ cache: { isr: true, isrRevalidate: 60 } }),
		] as never)
	...
	it("isr dynamic with revalidate → mode isr", () => {
		const routes = buildPrerenderRoutes([
			makeDef({ cache: { isr: "dynamic", isrRevalidate: 300 } }),
		] as never)
		expect(routes[0]?.mode).toBe("isr")
	})
	...
	it("virtualPath with group segment stripped", () => {
		const routes = buildPrerenderRoutes([
			makeDef({ virtualPath: "_root_/(marketing)/pricing" }),
		] as never)
	...
	it("isrRevalidate preserved", () => {
		const routes = buildPrerenderRoutes([
			makeDef({ cache: { isr: true, isrRevalidate: 300 } }),
		] as never)
	...
	it("ssgDefer preserved", () => {
		const routes = buildPrerenderRoutes([
			makeDef({ cache: { ssg: true, ssgDefer: "stream" } }),
		] as never)

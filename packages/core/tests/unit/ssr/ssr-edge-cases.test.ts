...
import { buildFlareStateScript, deriveStatus, type FlareState, serializeFlareState } from "../../../src/ssr/index.tsx";
...
describe("deriveStatus edge cases", () => {
	...
	it("401 wins over 403", () => {
		expect(deriveStatus([{ error: new UnauthorizedError("no") }, { error: new UnauthenticatedError("login") }])).toBe(
			401,
		);
	});

	it("401 wins over 404", () => {
		expect(deriveStatus([{ error: new NotFoundError("gone") }, { error: new UnauthenticatedError("login") }])).toBe(
			401,
		);
	});

	it("401 wins over 500", () => {
		expect(deriveStatus([{ error: new Error("crash") }, { error: new UnauthenticatedError("login") }])).toBe(401);
	});

	it("403 wins over 404", () => {
		expect(deriveStatus([{ error: new NotFoundError("gone") }, { error: new UnauthorizedError("no") }])).toBe(403);
	});

	it("403 wins over 500", () => {
		expect(deriveStatus([{ error: new Error("crash") }, { error: new UnauthorizedError("no") }])).toBe(403);
	});

	it("404 wins over 500", () => {
		expect(deriveStatus([{ error: new Error("crash") }, { error: new NotFoundError("gone") }])).toBe(404);
	});
	...
	it("multiple 500-class errors still returns 500", () => {
		expect(deriveStatus([{ error: new Error("a") }, { error: new Error("b") }, { error: new Error("c") }])).toBe(500);
	});

...
describe("serializeFlareState", () => {
	...
	it("escapes all < chars including <!-- and </style>", () => {
		const state = makeState({
			m: [{ d: "<!-- comment --> </style> <img>", h: undefined, i: "m1", p: undefined, v: "_root_" }],
		});
		const result = serializeFlareState(state);
		expect(result).not.toContain("<");
		expect(result).toContain("\\u003c");
...
describe("mergeHeadConfigs", () => {
	...
	it("jsonLd concat", () => {
		const result = mergeHeadConfigs({ jsonLd: [{ "@type": "WebSite" }] }, { jsonLd: [{ "@type": "Article" }] });
...
describe("mergeResponseHeaders", () => {
	...
	it("case-sensitive: Content-Type and content-type are different keys", () => {
		const result = mergeResponseHeaders({ "Content-Type": "text/html" }, { "content-type": "text/plain" });
...
describe("renderHeadToHtml", () => {
	...
	it("custom.scripts → <script nonce> with attributes and children", () => {
		const html = renderHeadToHtml({ custom: { scripts: [{ children: "console.log(1)", type: "module" }] } }, "abc");
...
describe("deriveStatus", () => {
	...
	it("mixed errors → highest priority wins (401 > 403 > 404 > 500)", () => {
		expect(deriveStatus([{ error: new NotFoundError() }, { error: new UnauthorizedError() }])).toBe(403);
	});

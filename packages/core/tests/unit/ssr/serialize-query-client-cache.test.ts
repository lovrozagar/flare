...
describe("serializeQueryClientCache: real QueryClient", () => {
	it("serializes data from a real TanStack QueryClient", () => {
		const { QueryClient } = require("@tanstack/solid-query") as typeof import("@tanstack/solid-query");
...
function parseEntries(script: string): Array<{ data: unknown; key: unknown[]; staleTime?: number }> {
	if (!script) return [];

...
describe("rewrite patterns — real world scenarios", () => {
	...
	it("lowercase normalization: case-insensitive routing via rewrite", () => {
		...
		expect(executeRewriteInput(rewrite, new URL("http://localhost/About/Team")).pathname).toBe("/about/team");
		expect(executeRewriteInput(rewrite, new URL("http://localhost/about/team")).pathname).toBe("/about/team");
	});
	...
	it("vanity URL: map multiple aliases to same target", () => {
		...
		expect(executeRewriteInput(rewrite, new URL("http://localhost/pricing")).pathname).toBe("/plans");
		expect(executeRewriteInput(rewrite, new URL("http://localhost/careers")).pathname).toBe("/about/jobs");
		expect(executeRewriteInput(rewrite, new URL("http://localhost/normal")).pathname).toBe("/normal");
	});

...
const testLoader: ImageLoader = ({ quality, src, width }) =>
	`https://cdn.test/w${width}/q${quality}${src}`
...
describe("responsive mode", () => {
	it("renders img with maxWidth as width attr and computed height", () => {
		dispose = render(
			() => <Image alt="hero" maxHeight={800} maxWidth={1200} src="/hero.jpg" />,
			container,
		)
	...
	it("computes height from aspectRatio", () => {
		dispose = render(
			() => <Image alt="wide" aspectRatio={16 / 9} maxWidth={1600} src="/wide.jpg" />,
			container,
		)
		const img = container.querySelector("img")
	...
	it("auto-generates sizes: (min-width: Xpx) Xpx, 100vw", () => {
		dispose = render(
			() => <Image alt="photo" maxHeight={600} maxWidth={900} src="/photo.jpg" />,
			container,
		)
		const img = container.querySelector("img")
...
describe("fixed mode", () => {
	it("renders with exact width/height attrs", () => {
		dispose = render(
			() => <Image alt="icon" fixed height={48} src="/icon.png" width={48} />,
			container,
		)
		const img = container.querySelector("img")
...
describe("priority", () => {
	it("sets eager/high/sync", () => {
		dispose = render(
			() => <Image alt="hero" maxHeight={600} maxWidth={1200} priority src="/hero.jpg" />,
			container,
		)
		const img = container.querySelector("img")
...
describe("static image data", () => {
	...
	it("fixed mode with static data uses density srcset", () => {
		...
		dispose = render(
			() => <Image alt="icon" fixed height={48} src={staticIcon} width={48} />,
			container,
		)
		const img = container.querySelector("img")
		const srcset = img?.getAttribute("srcset") ?? ""
...
describe("static image edge cases", () => {
	...
	it("custom sizes overrides auto-generated sizes", () => {
		dispose = render(
			() => <Image alt="hero" sizes="(max-width: 768px) 100vw, 50vw" src={staticHero} />,
			container,
		)
		const img = container.querySelector("img")
		expect(img?.getAttribute("sizes")).toBe("(max-width: 768px) 100vw, 50vw")
	...
	it("explicit blurDataURL overrides static data blur", () => {
		...
		dispose = render(
			() => <Image alt="hero" blurDataURL={customBlur} src={staticHero} />,
			container,
		)
		const img = container.querySelector("img")
		const style = img?.getAttribute("style") ?? ""
	...
	it("user style (object) merged with static blur + layout", () => {
		dispose = render(
			() => <Image alt="hero" src={staticHero} style={{ "border-radius": "8px" }} />,
			container,
		)
		const img = container.querySelector("img")
		const style = img?.getAttribute("style") ?? ""
	...
	it("static fixed mode uses provided width/height, not data's", () => {
		dispose = render(
			() => <Image alt="icon" fixed height={48} src={staticHero} width={48} />,
			container,
		)
		const img = container.querySelector("img")
		expect(img?.getAttribute("width")).toBe("48")

...
describe("style merge ordering", () => {
	it("layout styles always present (responsive)", () => {
		dispose = render(() => <Image alt="test" maxHeight={600} maxWidth={900} src="/photo.jpg" />, container);
	...
	it("layout + user string merged", () => {
		dispose = render(
			() => <Image alt="test" maxHeight={600} maxWidth={900} src="/photo.jpg" style={{ border: "1px solid red" }} />,
			container,
		);
		const img = container.querySelector("img");
		const style = img?.getAttribute("style") ?? "";
...
describe("responsive mode deep", () => {
	it("loader receives maxWidth as width param", () => {
		...
		dispose = render(() => <Image alt="test" loader={spy} maxHeight={600} maxWidth={1200} src="/in.jpg" />, container);
...
describe("loader precedence", () => {
	it("no loader → src passthrough, no srcset", () => {
		dispose = render(() => <Image alt="test" maxHeight={100} maxWidth={200} src="/photo.jpg" />, container);
		const img = container.querySelector("img");
...
describe("buildStaticSrcSet", () => {
	...
	it("width mode — returns all variants sorted ascending with w descriptors", () => {
		...
		expect(result).toBe("/a-640.webp 640w, /a-828.webp 828w, /a-1080.webp 1080w, /a-1920.webp 1920w");
	});
	...
	it("density mode — closest variant picked when exact 2x missing", () => {
		const result = buildStaticSrcSet({ 100: "/a-100.webp", 48: "/a-48.webp", 64: "/a-64.webp" }, "density", 48);
	...
	it("width mode — all variants above cap returns undefined", () => {
		const result = buildStaticSrcSet({ 1080: "/a-1080.webp", 1920: "/a-1920.webp" }, "width", 1920, 200);
...
describe("static image mode interactions", () => {
	...
	it("static fixed — density srcset uses provided width, not data width", () => {
		dispose = render(() => <Image alt="thumb" fixed height={50} src={staticData} width={400} />, container);
		const img = container.querySelector("img");
		const srcset = img?.getAttribute("srcset") ?? "";

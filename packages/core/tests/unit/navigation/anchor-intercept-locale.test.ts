...
function makeCtx(overrides?: Partial<FlareProviderContext>): FlareProviderContext {
	...
	const ctx: FlareProviderContext = {
		...
		setNavigationPhase: (v: import("../../../src/outlet/types").NavigationPhase) => { navigationPhase = v },
		setNotFound: (v: boolean) => { notFound = v },
		setParams: (p) => { params = p },
		setSearch: (s) => { search = s },
		setViewTransition: (vt: import("../../../src/outlet/types").BrowserViewTransition | null) => { viewTransition = vt },
	...
	Object.defineProperty(ctx, "_setNavigate", { value: (fn: typeof navigateFn) => { navigateFn = fn } })
	Object.defineProperty(ctx, "_setPrefetch", { value: (fn: typeof prefetchFn) => { prefetchFn = fn } })
	...
	describe("cross-worker path not intercepted when locale allow-list rejects it", () => {
		it('<a href="/docs"> NOT intercepted: locales=["en"], tree has only [[locale]]', () => {
			...
			const mockLoadRouteModules = vi.fn<
				(pathname: string, routeTree: unknown, layouts: unknown) => Promise<LoadedRouteModules>

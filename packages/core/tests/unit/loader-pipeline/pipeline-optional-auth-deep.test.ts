...
describe("mixed auth modes — nested routes", () => {
	it("root optional + page required → throws when auth null", async () => {
		...
		await expect(
			runPipeline(makeConfig({ authenticateFn: authFn, routes: [root, page] })),
	...
	it("3 levels: root no-auth + layout optional + page required → throws", async () => {
		...
		await expect(
			runPipeline(makeConfig({ authenticateFn: authFn, routes: [root, layout, page] })),
		).rejects.toThrow(UnauthenticatedError)
	})
	...
	it("4 levels deep: mixed modes throughout → strictest wins", async () => {
		...
		await expect(runPipeline(makeConfig({ authenticateFn: authFn, routes }))).rejects.toThrow(
...
describe("error handling with optional auth", () => {
	...
	it("optional auth null → authorize throws → pipeline throws", async () => {
		...
		await expect(
			runPipeline(makeConfig({ authenticateFn: authFn, routes: [route] })),
		).rejects.toThrow("authorize crash")
	})
...
describe("authenticateMode explicitly true", () => {
	...
	it("authenticateMode: true behaves same as no mode (default required)", async () => {
		...
		await expect(
			runPipeline(makeConfig({ authenticateFn: authFn, routes: [withMode] })),
		).rejects.toThrow(UnauthenticatedError)

		await expect(
			runPipeline(makeConfig({ authenticateFn: authFn, routes: [withoutMode] })),
		).rejects.toThrow(UnauthenticatedError)
	})
})

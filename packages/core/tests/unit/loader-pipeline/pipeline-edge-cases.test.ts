...
describe("store cache intercept", () => {
	...
	it("cache miss → loader called, result stored", async () => {
		const setFn = vi.fn<(key: string, entry: { data: unknown; storedAt: number }, ttl?: number) => Promise<void>>(() =>
			Promise.resolve(),
		);
		const store = makeFlareStore({
			get: vi.fn(() => Promise.resolve(null)),
	...
	it("store as factory function → resolved with env", async () => {
		...
		const result = await runPipeline(makeConfig({ env: { DB: "test" }, routes: [route], store: factory }));

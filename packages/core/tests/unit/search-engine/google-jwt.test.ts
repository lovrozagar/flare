...
describe("getGoogleAccessToken", () => {
	it("sends correct token exchange request", async () => {
		...
		const mockFetch = vi.fn(async (url: string, init: RequestInit) => {
			...
			return new Response(JSON.stringify({ access_token: "mock-token", expires_in: 3600, token_type: "Bearer" }));
		});
	...
	it("throws on failed token exchange", async () => {
		...
		try {
			await expect(
				getGoogleAccessToken({ clientEmail: "test@test.iam.gserviceaccount.com", privateKey: pem }, "scope"),

...
describe("#4: head-client custom links event handler filtering", () => {
	...
	it("filters onclick from custom links", async () => {
		const { applyHeadConfig, clearRouteTracking, initRouteHierarchy } = await import("../../src/head-client/index.ts");
		clearRouteTracking();
		...
		const linkEl = mockDoc.elements.find((el) => el.tag === "link" && el.attrs["rel"] === "prefetch");

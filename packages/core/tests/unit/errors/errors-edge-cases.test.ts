...
describe("RedirectResponse protocol edge cases", () => {
	it("JAVASCRIPT: all caps → throws", () => {
		expect(() => new RedirectResponse({ href: "JAVASCRIPT:void(0)" })).toThrow(
			"Unsafe redirect URL",
		)
	})
	...
	it("ftp: protocol → blocked (only http/https allowed)", () => {
		expect(() => new RedirectResponse({ href: "ftp://files.example.com" })).toThrow(
			"Unsafe redirect URL",
		)
	})

	it("mailto: → blocked (only http/https allowed)", () => {
		expect(() => new RedirectResponse({ href: "mailto:user@example.com" })).toThrow(
			"Unsafe redirect URL",
		)
	})

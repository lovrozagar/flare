...
describe("resolvePathParams", () => {
	...
	describe("required catch-all", () => {
		...
		it("handles deep nesting", () => {
			expect(resolvePathParams("/docs/[...slug]", { slug: ["api", "auth", "tokens"] })).toBe("/docs/api/auth/tokens");
		});

		it("throws for missing catch-all", () => {
			expect(() => resolvePathParams("/docs/[...slug]", {})).toThrow("Missing required catch-all param: slug");
		});
	...
	describe("optional catch-all", () => {
		...
		it("handles deep segments", () => {
			expect(resolvePathParams("/[[...slug]]", { slug: ["intro", "getting-started"] })).toBe("/intro/getting-started");
		});
		...
		it("mid-path filled optional catch-all produces clean path", () => {
			expect(resolvePathParams("/prefix/[[...slug]]/suffix", { slug: ["a", "b"] })).toBe("/prefix/a/b/suffix");
		});
	...
	describe("combined optional + required", () => {
		it("both present", () => {
			expect(resolvePathParams("/[[locale]]/products/[id]", { id: "123", locale: "en" })).toBe("/en/products/123");
		});
	...
	describe("encoding", () => {
		...
		it("encodes search query param", () => {
			expect(resolvePathParams("/search/[query]", { query: "hello world" })).toBe("/search/hello%20world");
		});

		it("encodes catch-all with spaces", () => {
			expect(resolvePathParams("/docs/[...slug]", { slug: ["hello world", "test"] })).toBe("/docs/hello%20world/test");
		});
	});
...
describe("buildUrl", () => {
	...
	describe("with params", () => {
		...
		it("resolves catch-all", () => {
			expect(buildUrl({ params: { slug: ["api", "auth"] }, to: "/docs/[...slug]" })).toBe("/docs/api/auth");
		});
	});

	describe("with search", () => {
		it("appends search params", () => {
			expect(buildUrl({ search: { page: 2, sort: "name" }, to: "/products" })).toBe("/products?page=2&sort=name");
		});
	...
	describe("with hash", () => {
		it("appends hash", () => {
			expect(buildUrl({ hash: "reviews", params: { id: "123" }, to: "/products/[id]" })).toBe("/products/123#reviews");
		});
	...
	describe("optional params", () => {
		...
		it("optional missing + required", () => {
			expect(buildUrl({ params: { id: "123" }, to: "/[[locale]]/products/[id]" })).toBe("/products/123");
		});
	});
	...
	describe("edge cases", () => {
		...
		it("unicode in search params", () => {
			expect(buildUrl({ search: { q: "日本語" }, to: "/search" })).toBe("/search?q=%E6%97%A5%E6%9C%AC%E8%AA%9E");
		});

		it("special chars in search key", () => {
			expect(buildUrl({ search: { "key with space": "val" }, to: "/" })).toBe("/?key%20with%20space=val");
		});

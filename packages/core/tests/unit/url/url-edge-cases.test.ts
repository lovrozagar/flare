...
describe("resolvePathParams optional catch-all", () => {
	...
	it("multiple segments with encoding", () => {
		expect(resolvePathParams("/docs/[[...path]]", { path: ["a b", "c/d"] })).toBe("/docs/a%20b/c%2Fd");
	});

	it("array with undefined items filtered", () => {
		expect(resolvePathParams("/docs/[[...path]]", { path: ["a", undefined, "b"] })).toBe("/docs/a/b");
	});
});
...
describe("resolvePathParams required catch-all", () => {
	...
	it("non-array → throws", () => {
		expect(() => resolvePathParams("/docs/[...path]", { path: "single" })).toThrow("must be an array");
	});
...
describe("resolvePathParams required single errors", () => {
	...
	it("array for single param → throws", () => {
		expect(() => resolvePathParams("/users/[id]", { id: ["a", "b"] })).toThrow("must be string, got array");
	});
});
...
describe("resolvePathParams encoding", () => {
	...
	it("unicode param value", () => {
		expect(resolvePathParams("/users/[name]", { name: "日本語" })).toBe(`/users/${encodeURIComponent("日本語")}`);
	});

...
describe("dedupe", () => {
	...
	it("cache per-request (not global)", async () => {
		...
		await runWithServerContext(
			{ nonce: "a", request: new Request("http://localhost/") },
			async () => {
				await deduped()
			},
		)
		...
		await runWithServerContext(
			{ nonce: "b", request: new Request("http://localhost/") },
			async () => {
				await deduped()
			},
		)
...
describe("fetch deduplication", () => {
	...
	it("GET with Request object input → deduplicated", async () => {
		...
		await runWithServerContext(
			...
			async () => {
				await Promise.all([
					fetch(new Request("http://example.com/api")),
					fetch(new Request("http://example.com/api")),
				])
			},
		)
	...
	it("GET with URL object input → deduplicated", async () => {
		...
		await runWithServerContext(
			...
			async () => {
				await Promise.all([
					fetch(new URL("http://example.com/api")),
					fetch(new URL("http://example.com/api")),
				])
			},
		)

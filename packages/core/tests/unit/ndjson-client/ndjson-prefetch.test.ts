...
describe("prefetch mode — raw deferred markers", () => {
	...
	it("prefetch fetch → hasDeferredMarkers absent for matches without deferred", async () => {
		const fetchSpy = vi
			.fn()
			.mockResolvedValue(
				createMockResponse([line({ d: { name: "test", value: 42 }, m: "route-1", t: "l" }), line({ t: "d" })]),
			);
		vi.stubGlobal("fetch", fetchSpy);
	...
	it("non-prefetch fetch → loaderData hydrated normally (no hasDeferredMarkers)", async () => {
		const fetchSpy = vi
			.fn()
			.mockResolvedValue(createMockResponse([line({ d: { name: "test" }, m: "route-1", t: "l" }), line({ t: "d" })]));
		vi.stubGlobal("fetch", fetchSpy);
	...
	it("prefetch with nested deferred markers → detected", async () => {
		...
		const fetchSpy = vi
			.fn()
			.mockResolvedValue(createMockResponse([line({ d: nested, m: "route-1", t: "l" }), line({ t: "d" })]));
		vi.stubGlobal("fetch", fetchSpy);
	...
	it("prefetch with null loaderData → hasDeferredMarkers absent", async () => {
		const fetchSpy = vi
			.fn()
			.mockResolvedValue(createMockResponse([line({ d: null, m: "route-1", t: "l" }), line({ t: "d" })]));
		vi.stubGlobal("fetch", fetchSpy);
	...
	it("prefetch with top-level deferred marker → detected", async () => {
		const fetchSpy = vi
			.fn()
			.mockResolvedValue(
				createMockResponse([line({ d: { __deferred: true, key: "d0" }, m: "route-1", t: "l" }), line({ t: "d" })]),
			);
		vi.stubGlobal("fetch", fetchSpy);
	...
	it("prefetch with __deferred: false → not a marker", async () => {
		const fetchSpy = vi
			.fn()
			.mockResolvedValue(
				createMockResponse([line({ d: { __deferred: false, key: "d0" }, m: "route-1", t: "l" }), line({ t: "d" })]),
			);
		vi.stubGlobal("fetch", fetchSpy);
	...
	it("prefetch with empty object → hasDeferredMarkers absent", async () => {
		const fetchSpy = vi
			.fn()
			.mockResolvedValue(createMockResponse([line({ d: {}, m: "route-1", t: "l" }), line({ t: "d" })]));
		vi.stubGlobal("fetch", fetchSpy);
	...
	it("prefetch with string/number loaderData → hasDeferredMarkers absent", async () => {
		const fetchSpy = vi
			.fn()
			.mockResolvedValue(createMockResponse([line({ d: "hello", m: "route-1", t: "l" }), line({ t: "d" })]));
		vi.stubGlobal("fetch", fetchSpy);

...
describe("malformed JSON in NDJSON stream", () => {
	...
	it("empty lines are silently skipped", async () => {
		const fetchSpy = vi
			.fn()
			.mockResolvedValue(
				createMockResponse(["", line({ d: "data1", m: "m1", t: "l" }), "   ", line({ t: "d" })]),
			)
		vi.stubGlobal("fetch", fetchSpy)
	...
	it("JSON array instead of object → skipped", async () => {
		const fetchSpy = vi
			.fn()
			.mockResolvedValue(
				createMockResponse(["[1, 2, 3]", line({ d: "data", m: "m1", t: "l" }), line({ t: "d" })]),
			)
		vi.stubGlobal("fetch", fetchSpy)
	...
	it("JSON null → skipped", async () => {
		const fetchSpy = vi
			.fn()
			.mockResolvedValue(
				createMockResponse(["null", line({ d: "data", m: "m1", t: "l" }), line({ t: "d" })]),
			)
		vi.stubGlobal("fetch", fetchSpy)
...
describe("redirect in NDJSON stream", () => {
	...
	it("redirect with xl=true sets external=true and url=href", async () => {
		const fetchSpy = vi
			.fn()
			.mockResolvedValue(
				createMockResponse([line({ s: 301, t: "x", u: "https://external.com", xl: true })]),
			)
		vi.stubGlobal("fetch", fetchSpy)
...
describe("error reconstruction", () => {
	...
	it("error with no name uses generic Error", async () => {
		const fetchSpy = vi
			.fn()
			.mockResolvedValue(
				createMockResponse([line({ e: { message: "oops" }, m: "m1", t: "e" }), line({ t: "d" })]),
			)
		vi.stubGlobal("fetch", fetchSpy)
	...
	it("error with no e field → Unknown error message", async () => {
		const fetchSpy = vi
			.fn()
			.mockResolvedValue(createMockResponse([line({ m: "m1", t: "e" }), line({ t: "d" })]))
		vi.stubGlobal("fetch", fetchSpy)
...
describe("head messages in stream", () => {
	it("head message with valid object adds to perRouteHeads", async () => {
		const fetchSpy = vi
			.fn()
			.mockResolvedValue(
				createMockResponse([line({ d: { title: "Page" }, m: "m1", t: "h" }), line({ t: "d" })]),
			)
		vi.stubGlobal("fetch", fetchSpy)
	...
	it("head message with non-object d → ignored", async () => {
		const fetchSpy = vi
			.fn()
			.mockResolvedValue(
				createMockResponse([line({ d: "string-not-object", m: "m1", t: "h" }), line({ t: "d" })]),
			)
		vi.stubGlobal("fetch", fetchSpy)
...
describe("prefetch mode edge cases", () => {
	...
	it("prefetch mode with preloaderContext preserved", async () => {
		const fetchSpy = vi
			...
			.mockResolvedValue(
				createMockResponse([
					line({ d: "data", m: "m1", p: { userId: "123" }, t: "l" }),
					line({ t: "d" }),
				]),
			)
		vi.stubGlobal("fetch", fetchSpy)
...
describe("ready vs done signal ordering", () => {
	...
	it("done without ready still resolves (done also calls resolveLoadersReady)", async () => {
		const fetchSpy = vi
			.fn()
			.mockResolvedValue(
				createMockResponse([line({ d: "data", m: "m1", t: "l" }), line({ t: "d" })]),
			)
		vi.stubGlobal("fetch", fetchSpy)

...
describe("B4: NDJSON post-loaders error warning", () => {
	it("non-redirect error after loadersReady calls warn()", async () => {
		...
		const chunk1 = encode(`${line({ d: { value: 1 }, m: "match1", t: "l" })}\n${line({ t: "r" })}\n`);
		...
		expect(warnSpy).toHaveBeenCalledWith("[flare:ndjson]", "post-loaders stream error", expect.any(Error));
	});
	...
	it("redirect after loadersReady still captured as pendingRedirect", async () => {
		...
		await expect(fetchNDJSON({ url: "http://localhost/test" })).rejects.toBeInstanceOf(RedirectResponse);
	});
});

		...
		const handler = createServerHandler(
			makeConfig({
				middlewareEntries: [
					{
						middlewares: [
							(ctx) => Promise.resolve(ctx.bypass(new Response("bypassed", { status: 200 }))),
						],
					},
				],
			}),
		...
		const handler = createServerHandler(
			makeConfig({
				middlewareEntries: [
					{
						middlewares: [
							(ctx) => Promise.resolve(ctx.respond(new Response("responded", { status: 200 }))),
						],
					},
				],
			}),
...
describe("matched route — data request (x-d:1)", () => {
	function makeHandlerWithRoute(
		routeOverrides?: Partial<RouteData>,
		configOverrides?: Partial<ServerHandlerConfig>,
	) {
		const tree = makeTreeWithRoute("/about", makeRouteData(routeOverrides))
		const router = makeRouter({ layouts: {}, routeTree: tree })

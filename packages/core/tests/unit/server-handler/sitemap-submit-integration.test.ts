		...
		const handler = createServerHandler(
			makeConfig({
				middlewareEntries: [
					{
						middlewares: [
							(ctx) => Promise.resolve(ctx.bypass(new Response("blocked", { status: 403 }))),
						],
					},
				],
			}),
		...
		const handler = createServerHandler(
			makeConfig({
				middlewareEntries: [
					{
						middlewares: [
							(ctx) => Promise.resolve(ctx.respond(new Response("intercepted", { status: 200 }))),
						],
					},
				],
			}),
...
describe("sitemap submit — CSP nonce", () => {
	...
	it("CSP overrides applied to sitemap response", async () => {
		const handler = createServerHandler(
			makeConfig({ csp: { "frame-ancestors": ["https://trusted.com"] } }),
		)
		const res = await handler.fetch(makeSubmitRequest(), {})
		const csp = res.headers.get("content-security-policy")

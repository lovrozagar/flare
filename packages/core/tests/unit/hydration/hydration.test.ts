...
describe("buildMatches", () => {
	...
	it("layouts ordered before page", () => {
		...
		const modules = {
			layouts: [
				{ _type: "layout" as const, render: () => null, variablePath: "", virtualPath: "_root_" },
			],
			page: {
				_type: "render" as const,
				render: () => null,
...
describe("extractRootBoundaries", () => {
	...
	it("root with builder-marked fns → filtered out", () => {
		...
		const result = extractRootBoundaries(
			root as unknown as import("../../../src/navigation/types").LoadedRouteModule,
		)

...
import { createTrackedQueryClient, hydrateQueryCache, type QueryState } from "../../../src/query-client/index.tsx";
...
describe("tracked vs serialize: data format compatibility", () => {
	...
	it("serialize does not see tracked staleTime from defaults", () => {
		...
		const serializedEntries = parseScript(serializeQueryClientCache(qc, "n"));
		const serializedEntry = serializedEntries.find((e) => JSON.stringify(e.key) === JSON.stringify(["with-defaults"]));

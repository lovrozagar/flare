...
describe("runGenerate — fsVirtualPaths on (conforming trees)", () => {
	...
	it("simple: root layout + home + about", () => {
		...
		expect(readFileSync(join(tmpDir, "src/routes/_root_/home.page.tsx"), "utf-8")).toContain(`createPage("_root_/")`);
	});
	...
	it("four nested group layouts + index + sibling page", () => {
		...
		writeFile(tmpDir, "src/routes/_root_/(dc)/(inner)/(deep)/(leaf)/deep-cache/uncached/uncached.page.tsx", "");
...
describe("runGenerate — fsVirtualPaths on (bad structure)", () => {
	...
	it("mixed leftover about.tsx is a warning — valid suffix routes still generate", () => {
		...
		try {
			...
			expect(warns.some((w) => w.includes("[flare:generate]") && w.includes("about.tsx:3"))).toBe(true);
		} finally {
			console.warn = orig;
		}
		...
		expect(readFileSync(join(tmpDir, "src/routes/about.tsx"), "utf-8")).toContain(`createPage("_root_/about")`);
	});
...
describe("runGenerate — fsVirtualPaths off (string-style)", () => {
	...
	it("keeps handwritten virtual paths and does not require suffixes", () => {
		...
		writeFile(tmpDir, "src/routes/about.tsx", `export const route = createPage("_root_/about")\n`);
		writeFile(tmpDir, "src/routes/dashboard/_layout_.tsx", `export const route = createLayout("_root_/(dashboard)")\n`);
		writeFile(tmpDir, "src/routes/dashboard/index.tsx", `export const route = createPage("_root_/(dashboard)/")\n`);
		writeFile(tmpDir, "src/routes/users/[id].tsx", `export const route = createPage("_root_/users/[id]")\n`);
		writeFile(tmpDir, "src/routes/files/[...path].tsx", `export const route = createPage("_root_/files/[...path]")\n`);
	...
	it("does not rewrite a mismatched path when fs is off", () => {
		writeFile(tmpDir, "src/routes/contact.tsx", `export const route = createPage("_root_/old-name")\n`);
		runGenerate({ fsCodegen: false, rootDir: tmpDir });
		expect(readFileSync(join(tmpDir, "src/routes/contact.tsx"), "utf-8")).toContain(`createPage("_root_/old-name")`);
		expect(readFileSync(join(tmpDir, "src/_gen/routes.gen.ts"), "utf-8")).toContain('x: "_root_/old-name"');
	});
...
describe("consumers — fs on vs off", () => {
	...
	it("flare({ codegen: { fsVirtualPaths: true } }) warns on leftover string-style and still generates", async () => {
		...
		process.chdir(root);
		try {
			const { flare } = await import("@lovrozagar/flare/plugins");
		...
		expect(readFileSync(join(root, "src/routes/orphan.tsx"), "utf-8")).toContain(`createPage("_root_/orphan")`);
	});

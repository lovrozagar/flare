...
describe("extractCacheFromChain", () => {
	...
	it("combined client timing + prefetch", () => {
		const c = extractCacheFromChain(`.cache({ client: { staleTime: 30000, gcTime: 60000, prefetch: "intent" } })`);
		expect(c.client?.staleTime).toBe(30000);
		expect(c.client?.gcTime).toBe(60000);
		expect(c.client?.prefetch).toBe("intent");
	});
	...
	it("prefetchStaleTime + prefetchGcTime extracted", () => {
		const c = extractCacheFromChain(".cache({ client: { prefetchStaleTime: 10000, prefetchGcTime: 5000 } })");
	...
	it("isr: { params: () => [...], revalidate: 60 } → isr dynamic", () => {
		const c = extractCacheFromChain(`.cache({ isr: { params: async () => [{ slug: "a" }], revalidate: 60 } })`);
	...
	it("isr: { dynamicParams: false } extracted", () => {
		const c = extractCacheFromChain(".cache({ isr: { dynamicParams: false, params: () => [], revalidate: 60 } })");
		expect(c.isrDynamicParams).toBe(false);
	});

	it("isr: { dynamicParams: true } extracted", () => {
		const c = extractCacheFromChain(".cache({ isr: { dynamicParams: true, params: () => [], revalidate: 60 } })");
	...
	it("mixed: duration string client + numeric isr revalidate", () => {
		const c = extractCacheFromChain(`.cache({ client: { staleTime: "10m" }, isr: { revalidate: 3600 } })`);
		expect(c.client?.staleTime).toBe(600000);
		expect(c.isrRevalidate).toBe(3600);
	});

	it("mixed: numeric client + duration string isr revalidate", () => {
		const c = extractCacheFromChain(`.cache({ client: { staleTime: 30000 }, isr: { revalidate: "5m" } })`);
		expect(c.client?.staleTime).toBe(30000);
		expect(c.isrRevalidate).toBe(300);
	});
...
describe("extractRouteDefinitions", () => {
	...
	it("multiple routes in one file → all extracted", () => {
		const src = [`export const PageA = createPage("_root_/a")`, `export const PageB = createPage("_root_/b")`].join(
			"\n",
		);
...
describe("generateRoutesFile", () => {
	it("produces valid file with type imports and static tree", () => {
		...
		expect(file).toContain(`import type { RouteData, TreeNode } from "@lovrozagar/flare/codegen"`);
	...
	it("emits S helper only when tree has non-empty children", () => {
		const flat: RouteDefinition[] = [makeDef({ exportName: "A", filePath: "routes/a.ts", virtualPath: "_root_/a" })];
...
describe("scanSourceFiles", () => {
	...
	it("finds .ts route files", () => {
		writeFileSync(join(tmpDir, "src", "routes", "home.ts"), `export const HomePage = createPage("_root_/")`);
	...
	it("finds .tsx route files", () => {
		writeFileSync(join(tmpDir, "src", "routes", "about.tsx"), `export const AboutPage = createPage("_root_/about")`);
		const defs = scanSourceFiles({ rootDir: tmpDir });
		expect(defs).toHaveLength(1);
	});

	it("skips _gen/ directory", () => {
		writeFileSync(join(tmpDir, "src", "_gen", "routes.gen.ts"), `export const X = createPage("_root_/x")`);
		const defs = scanSourceFiles({ rootDir: tmpDir });
		expect(defs).toHaveLength(0);
	});

	it("skips node_modules", () => {
		writeFileSync(join(tmpDir, "src", "node_modules", "pkg", "index.ts"), `export const X = createPage("_root_/x")`);
		const defs = scanSourceFiles({ rootDir: tmpDir });
		expect(defs).toHaveLength(0);
	});

	it("skips *.gen.ts files", () => {
		writeFileSync(join(tmpDir, "src", "routes", "types.gen.ts"), `export const X = createPage("_root_/x")`);
		const defs = scanSourceFiles({ rootDir: tmpDir });
		expect(defs).toHaveLength(0);
	});
	...
	it("skips dirs starting with ignorePrefix", () => {
		...
		writeFileSync(join(tmpDir, "src", "__ignored", "test.ts"), `export const X = createPage("_root_/x")`);
...
describe("runGenerate", () => {
	...
	it("generates routes.gen.ts from source files", () => {
		writeFileSync(join(tmpDir, "src", "routes", "home.ts"), `export const HomePage = createPage("_root_/")`);
		writeFileSync(join(tmpDir, "src", "routes", "_root_.ts"), `export const RootLayout = createRootLayout("_root_")`);
	...
	it("throws on duplicate virtualPaths", () => {
		writeFileSync(join(tmpDir, "src", "routes", "a.ts"), `export const A = createPage("_root_/dup")`);
		writeFileSync(join(tmpDir, "src", "routes", "b.ts"), `export const B = createPage("_root_/dup")`);
...
describe("extractParamsFromPattern", () => {
	...
	it("catch-all → string[] type", () => {
		expect(extractParamsFromPattern("/catch-all/[...segments]")).toEqual([{ name: "segments", type: "string[]" }]);
	});
	...
	it("optional single param → string | undefined type", () => {
		expect(extractParamsFromPattern("/[[locale]]")).toEqual([{ name: "locale", type: "string | undefined" }]);
	});

	it("optional single param with static children", () => {
		expect(extractParamsFromPattern("/[[locale]]/about")).toEqual([{ name: "locale", type: "string | undefined" }]);
	});
...
describe("generateRouteRegistry", () => {
	...
	it("optional catch-all → string[] | undefined param", () => {
		...
		expect(result).toContain('"/locale/[[...locale]]": { params: { locale: string[] | undefined } }');
	});

	it("layouts excluded from routes section", () => {
		const result = generateRouteRegistry([makeDef("_root_/(auth)", "layout"), makeDef("_root_/about")], "src/_gen");
		expect(result).toContain('"/about"');
	...
	it("multiple params sorted in type", () => {
		const result = generateRouteRegistry([makeDef("_root_/users/[userId]/posts/[postId]")], "src/_gen");
	...
	it("output is valid declare module augmentation", () => {
		const result = generateRouteRegistry([makeDef("_root_/about")], "src/_gen");
		expect(result).toContain('declare module "@lovrozagar/flare/codegen"');
		expect(result).toContain("interface FlareRegister");
	});
	...
	it("emits routeSearchParams for pages with hasInput", () => {
		...
		expect(result).toContain('"/products": InferSearchParams<typeof import("../routes/products")["route"]>');
	...
	it("emits routeParams for pages with hasInput", () => {
		...
		expect(result).toContain('"/validated/[id]": InferParams<typeof import("../routes/validated/[id]")["route"]>');
...
describe("computeParentLayouts", () => {
	...
	it("page under nested layout → root + layout sorted by depth", () => {
		expect(computeParentLayouts("_root_/(blog)/blog/[slug]", layoutPaths)).toEqual(["_root_", "_root_/(blog)"]);
	});
	...
	it("deeply nested → full chain", () => {
		...
		expect(computeParentLayouts("_root_/(a)/(b)/page", deep)).toEqual(["_root_", "_root_/(a)", "_root_/(a)/(b)"]);
	});
});
...
describe("extractInterceptFromChain", () => {
	...
	it("multiple from paths", () => {
		const c = extractInterceptFromChain(`.intercept({ from: ["/products", "/search"], render: "panel" })`);

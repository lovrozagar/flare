...
describe("deriveVirtualPath", () => {
	...
	it("nested page with dynamic param", () => {
		expect(deriveVirtualPath("routes/_root_/blog/[slug]/post.page.tsx", "routes")).toBe("_root_/blog/[slug]");
	});

	it("layout in group", () => {
		expect(deriveVirtualPath("routes/_root_/(blog)/blog.layout.tsx", "routes")).toBe("_root_/(blog)");
	});
	...
	it("deep nesting with groups", () => {
		expect(deriveVirtualPath("routes/_root_/(dc)/(inner)/(deep)/deep-cache/x.page.tsx", "routes")).toBe(
			"_root_/(dc)/(inner)/(deep)/deep-cache",
		);
	});

	it("multi-root layout scope", () => {
		expect(deriveVirtualPath("routes/_admin_/dashboard/dash.page.tsx", "routes")).toBe("_admin_/dashboard");
	});

	it("dot segments", () => {
		expect(deriveVirtualPath("routes/_root_/sitemap.xml/x.page.tsx", "routes")).toBe("_root_/sitemap.xml");
	});

	it("catch-all param", () => {
		expect(deriveVirtualPath("routes/_root_/docs/[...slug]/x.page.tsx", "routes")).toBe("_root_/docs/[...slug]");
	});
	...
	it("escaped underscore does not trigger ignore", () => {
		expect(deriveVirtualPath("routes/_root_/[_]internal/api/x.page.tsx", "routes")).toBe("_root_/_internal/api");
	});

	it("layout with nested path dir", () => {
		expect(deriveVirtualPath("routes/_root_/(products)/products/[id]/detail.page.tsx", "routes")).toBe(
			"_root_/(products)/products/[id]",
		);
	});
	...
	it("layout file derives to its directory", () => {
		expect(deriveVirtualPath("routes/_root_/(auth)/auth.layout.tsx", "routes")).toBe("_root_/(auth)");
	});
	...
	it("deep nested layout stacking", () => {
		expect(
			deriveVirtualPath("routes/_root_/(deep-cache)/(dc-inner)/(dc-deep)/(dc-leaf)/leaf.layout.tsx", "routes"),
...
describe("scanSourceFilesFsCodegen", () => {
	...
	it("discovers multiple route files", () => {
		writeFile("src/routes/_root_/about/about.page.tsx", `export const route = createPage("_root_/about")`);
		writeFile("src/routes/_root_/(blog)/blog.layout.tsx", `export const route = createLayout("_root_/(blog)")`);
		writeFile(
	...
	it("root-layout stays _root_ while page gets _root_/", () => {
		writeFile("src/routes/_root_/root.root-layout.tsx", `export const route = createRootLayout("_root_")`);
		writeFile("src/routes/_root_/home.page.tsx", `export const route = createPage("_root_/")`);
...
describe("writeRouteDeclaration", () => {
	...
	it("writes createPage declaration into empty file", () => {
		...
		expect(content).toContain(`import { createPage } from "@lovrozagar/flare/page"`);
	...
	it("writes createLayout declaration", () => {
		...
		expect(content).toContain(`import { createLayout } from "@lovrozagar/flare/layout"`);
	...
	it("writes createRootLayout declaration", () => {
		...
		expect(content).toContain(`import { createRootLayout } from "@lovrozagar/flare/root-layout"`);
...
describe("validateRouteDefinitions", () => {
	...
	it("detects catch-all shadowing specific pages", () => {
		const errors = validateRouteDefinitions([
			...
			makeDef("_root_/docs/getting-started", "page", "src/routes/_root_/docs/getting-started/x.page.tsx"),
		]);
		expect(errors).toHaveLength(1);
	...
	it("detects optional catch-all shadowing specific pages", () => {
		const errors = validateRouteDefinitions([
			makeDef("_root_/locale/[[...locale]]", "page", "src/routes/_root_/locale/[[...locale]]/x.page.tsx"),
...
describe("runGenerate skip-write", () => {
	...
	it("fsCodegen rejects duplicate pages at same virtual path", () => {
		mkdirSync(join(tmpDir, "src", "routes", "_root_"), { recursive: true });
		writeFileSync(join(tmpDir, "src", "routes", "_root_", "a.page.tsx"), `export const route = createPage("_root_/")`);
		writeFileSync(join(tmpDir, "src", "routes", "_root_", "b.page.tsx"), `export const route = createPage("_root_/")`);

...
describe("ExtractPathParams", () => {
	...
	it("multiple params → intersection", () => {
		expectTypeOf<ExtractPathParams<"_root_/store/[storeSlug]/customers/[customerId]">>().toEqualTypeOf<{
			customerId: string;
			storeSlug: string;
		}>();
	});
...
type MockLayoutWithPreloader = {
	_type: "layout";
	render: (props: { children: unknown; loaderData: void; preloaderContext: { theme: string } }) => unknown;
	virtualPath: "_root_";
};
...
type MockPageDefaultSearch = {
	_type: "render";
	render: (props: { loaderData: void; location: { search: Record<string, string> }; preloaderContext: {} }) => unknown;
	virtualPath: "_root_/";
};
...
describe("ParentAuthResolution", () => {
	...
	it("true wins over optional (StrictestAuth)", () => {
		expectTypeOf<ParentAuthResolution<"_root_/(test-deep-auth)/(test-deep-opt)/leaf">>().toEqualTypeOf<true>();
	});

	it("optional wins over false", () => {
		expectTypeOf<ParentAuthResolution<"_root_/(test-deep-auth)/(test-deep-opt)">>().toEqualTypeOf<true>();
	});

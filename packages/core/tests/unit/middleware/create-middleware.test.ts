...
describe("virtualPath()", () => {
	...
	it("works as .use() matcher", () => {
		...
		const entries: MiddlewareEntry[] = [{ matchers: [virtualPath("/[[locale]]/dashboard")], middlewares: [mwA] }];
...
describe("resolveMiddlewareEntries()", () => {
	...
	it("includes entries where at least one matcher matches", () => {
		const entries: MiddlewareEntry[] = [{ matchers: [compilePattern("/dashboard/*")], middlewares: [mwA] }];

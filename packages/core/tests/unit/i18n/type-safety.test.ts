...
describe("ExtractICUVars", () => {
	...
	it("extracts variable from plural block", () => {
		expectTypeOf<
			ExtractICUVars<"{count, plural, one{# item} other{# items}}">
		>().toEqualTypeOf<"count">()
	})

	it("extracts variable from select block", () => {
		expectTypeOf<
			ExtractICUVars<"{gender, select, male{He} female{She} other{They}}">
		>().toEqualTypeOf<"gender">()
	})
	...
	it("extracts mixed simple and plural vars", () => {
		expectTypeOf<
			ExtractICUVars<"Hi {name}, {count, plural, one{# msg} other{# msgs}}">

...
describe("CSS scoping", () => {
	...
	it("@keyframes passed through unscoped", () => {
		registerCSSByName("test", "@keyframes spin { from { transform: rotate(0deg) } to { transform: rotate(360deg) } }");
		const css = getScopedStyles();
...
describe("findMatchingBrace — url() handling", () => {
	it("unquoted url() with braces does not break brace matching", () => {
		registerCSSByName("test", ".icon { background: url(data:image/svg+xml,%3Csvg%7B%7D%3E%3C/svg%3E); color: red }");
		const css = getScopedStyles();
		expect(css).toContain("color:red");

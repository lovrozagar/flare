...
describe("CSS refcount — invariant: never negative refcount", () => {
	it("removing route that never had CSS does not corrupt state", () => {
		...
		expect(mockDoc.elements.filter((e) => e.tag === "link" && e.attrs.href === "/only-r1.css")).toHaveLength(1);
...
describe("CSS refcount — route CSS swap", () => {
	it("route changes from /a.css to /b.css — old removed, new added", () => {
		...
		expect(mockDoc.elements.filter((e) => e.tag === "link" && e.attrs.href === "/a.css")).toHaveLength(1);
		expect(mockDoc.elements.filter((e) => e.tag === "link" && e.attrs.href === "/b.css")).toHaveLength(0);

		/* Swap CSS */
		applyPerRouteHeads([{ head: { css: "/b.css" }, matchId: "r1" }]);
		expect(mockDoc.elements.filter((e) => e.tag === "link" && e.attrs.href === "/a.css")).toHaveLength(0);
		expect(mockDoc.elements.filter((e) => e.tag === "link" && e.attrs.href === "/b.css")).toHaveLength(1);
	});
	...
	it("shared CSS swap: r1 swaps /shared→/new, r2 keeps /shared — shared persists", () => {
		...
		const linksBefore = mockDoc.elements.filter((e) => e.tag === "link" && e.attrs.href === "/shared.css");
		...
		expect(mockDoc.elements.filter((e) => e.tag === "link" && e.attrs.href === "/shared.css")).toHaveLength(1);
		expect(mockDoc.elements.filter((e) => e.tag === "link" && e.attrs.href === "/new.css")).toHaveLength(1);
	});
});
...
describe("CSS refcount — mixed CSS types in same route", () => {
	it("link + custom.styles in same route — link refcounted, style direct", () => {
		...
		expect(mockDoc.elements.filter((e) => e.tag === "link" && e.attrs.href === "/link.css")).toHaveLength(1);
	...
	it("css array + custom.links stylesheet in same route", () => {
		...
		expect(mockDoc.elements.filter((e) => e.tag === "link" && e.attrs.rel === "stylesheet")).toHaveLength(2);

		applyPerRouteHeads([]);
		expect(mockDoc.elements.filter((e) => e.tag === "link" && e.attrs.rel === "stylesheet")).toHaveLength(0);
	});
});
...
describe("CSS refcount — clearRouteTracking resets all state", () => {
	it("after clear, CSS state is fully reset", () => {
		...
		expect(mockDoc.elements.filter((e) => e.tag === "link" && e.attrs.href === "/fresh.css")).toHaveLength(1);
	});
});
...
describe("CSS refcount — navigation A→B→A→C", () => {
	it("shared CSS survives A→B, removed at C", () => {
		...
		expect(mockDoc.elements.filter((e) => e.tag === "link" && e.attrs.href === "/shared.css")).toHaveLength(1);
		expect(mockDoc.elements.filter((e) => e.tag === "link" && e.attrs.href === "/own.css")).toHaveLength(1);
		...
		expect(mockDoc.elements.filter((e) => e.tag === "link" && e.attrs.href === "/shared.css")).toHaveLength(1);
		expect(mockDoc.elements.filter((e) => e.tag === "link" && e.attrs.href === "/own.css")).toHaveLength(0);

		/* C: completely different route — shared removed */
		applyPerRouteHeads([{ head: { css: "/c.css" }, matchId: "r4" }]);
		expect(mockDoc.elements.filter((e) => e.tag === "link" && e.attrs.href === "/shared.css")).toHaveLength(0);
		expect(mockDoc.elements.filter((e) => e.tag === "link" && e.attrs.href === "/c.css")).toHaveLength(1);
	});
});

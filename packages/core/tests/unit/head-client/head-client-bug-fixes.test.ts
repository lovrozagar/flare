...
describe("Bug 10: OG image:type handling", () => {
	it("og:image:type tags created and cleaned up when updating OG images", () => {
		...
		expect(mockDoc.head.querySelector('meta[property="og:image:type"]')?.attrs.content).toBe(
			"image/webp",
		)
...
describe("Bug 19: OG video tags created and cleaned during SPA nav", () => {
	it("og:video meta tags created from openGraph.videos", () => {
		...
		expect(mockDoc.head.querySelector('meta[property="og:video:type"]')?.attrs.content).toBe(
			"video/mp4",
		)
		expect(mockDoc.head.querySelector('meta[property="og:video:width"]')?.attrs.content).toBe(
			"1280",
		)
		expect(mockDoc.head.querySelector('meta[property="og:video:height"]')?.attrs.content).toBe(
			"720",
		)
	})
...
describe("Bug 19: OG audio tags created and cleaned during SPA nav", () => {
	it("og:audio meta tags created from openGraph.audio", () => {
		...
		expect(mockDoc.head.querySelector('meta[property="og:audio"]')?.attrs.content).toBe(
			"https://example.com/song.mp3",
		)
		expect(mockDoc.head.querySelector('meta[property="og:audio:secure_url"]')?.attrs.content).toBe(
			"https://example.com/song.mp3",
		)
		expect(mockDoc.head.querySelector('meta[property="og:audio:type"]')?.attrs.content).toBe(
			"audio/mpeg",
		)
	})
})
...
describe("Bug 19: Twitter image tags created and cleaned during SPA nav", () => {
	it("twitter:image meta tags created from twitter.images", () => {
		...
		expect(mockDoc.head.querySelector('meta[name="twitter:image:alt"]')?.attrs.content).toBe(
			"Hero image",
		)
	})
...
describe("Bug 22: Top-level head.images handling", () => {
	it("top-level images creates og:image tags", () => {
		...
		expect(mockDoc.head.querySelector('meta[property="og:image:width"]')?.attrs.content).toBe(
			"1200",
		)
		expect(mockDoc.head.querySelector('meta[property="og:image:height"]')?.attrs.content).toBe(
			"630",
		)
		expect(mockDoc.head.querySelector('meta[property="og:image:type"]')?.attrs.content).toBe(
			"image/jpeg",
...
describe("Bug 28: head.meta fields during SPA nav", () => {
	...
	it("generic meta keys use correct HTML name mapping", () => {
		...
		expect(mockDoc.head.querySelector('meta[name="application-name"]')?.attrs.content).toBe(
			"My App",
		)
		...
		expect(mockDoc.head.querySelector('meta[name="mobile-web-app-capable"]')?.attrs.content).toBe(
			"yes",
		)
		expect(mockDoc.head.querySelector('meta[name="publisher"]')?.attrs.content).toBe(
			"Publisher Inc",
		)
	})
	...
	it("apple meta fields handled during SPA nav", () => {
		...
		expect(
			mockDoc.head.querySelector('meta[name="apple-mobile-web-app-capable"]')?.attrs.content,
		).toBe("yes")
		expect(
			mockDoc.head.querySelector('meta[name="apple-mobile-web-app-status-bar-style"]')?.attrs
				.content,
		).toBe("black-translucent")
		expect(
			mockDoc.head.querySelector('meta[name="apple-mobile-web-app-title"]')?.attrs.content,
		).toBe("My PWA")
	})
...
describe("Bug 29: custom.meta during SPA nav", () => {
	it("custom meta tags created from head.custom.meta", () => {
		...
		expect(mockDoc.head.querySelector('meta[name="google-site-verification"]')?.attrs.content).toBe(
			"abc123",
		)
...
describe("Bug 29: custom.links (non-stylesheet) during SPA nav", () => {
	it("custom non-stylesheet links created from head.custom.links", () => {
		...
		expect(mockDoc.head.querySelector('link[rel="preconnect"]')?.attrs.href).toBe(
...
describe("Bug 30: sized favicons during SPA nav", () => {
	...
	it("all three sized favicons created", () => {
		...
		expect(mockDoc.head.querySelector('link[rel="icon"][sizes="96x96"]')?.attrs.href).toBe(
			"/icons/icon-96.png",
		)
		expect(mockDoc.head.querySelector('link[rel="icon"][sizes="192x192"]')?.attrs.href).toBe(
			"/icons/icon-192.png",
		)
		expect(mockDoc.head.querySelector('link[rel="icon"][sizes="512x512"]')?.attrs.href).toBe(
			"/icons/icon-512.png",
		)
	})

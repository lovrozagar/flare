...
function createMockDocument() {
	const elements = new Map<string, { attrs: Record<string, string>; tag: string; textContent: string }>();
	let titleValue = "";
...
describe("OG image edge cases", () => {
	it("update from 3 images → 1 image → stale tags removed", () => {
		...
		expect(mockDoc.head.querySelector('meta[property="og:image"]')?.attrs.content).toBe("https://only.jpg");
	});
	...
	it("OG image with only url (no width/height/alt)", () => {
		...
		expect(mockDoc.head.querySelector('meta[property="og:image"]')?.attrs.content).toBe("https://plain.png");
...
describe("route hierarchy transitions", () => {
	it("3-level hierarchy → sibling nav → parent preserved, old child removed", () => {
		...
		expect(mockDoc.head.querySelector('meta[name="description"]')?.attrs.content).toBe("parent-desc");

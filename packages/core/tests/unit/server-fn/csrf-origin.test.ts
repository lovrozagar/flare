...
function createFns(
	overrides?: Partial<ServerFnHandlerRegistration>,
...
async function runInContext<T>(fn: () => T | Promise<T>): Promise<T> {
	return runWithServerContext(
		{ nonce: "test-nonce", request: new Request("http://localhost") },
		() => fn(),
	)
}
...
describe("handleServerFnRequest CSRF Origin validation", () => {
	...
	it("CSRF1: same-origin POST → 200", async () => {
		...
		const res = await runInContext(() =>
			handleServerFnRequest(postJson(FN_URL, "http://localhost"), {}, fns),
		)
	...
	it("CSRF3: same-origin https → 200", async () => {
		...
		const res = await runInContext(() =>
			handleServerFnRequest(postJson(url, "https://app.com"), {}, fns),
		)
		expect(res.status).toBe(200)
	})
	...
	it("CSRF5: cross-origin POST → 403", async () => {
		const fns = createFns()
		const res = await runInContext(() =>
			handleServerFnRequest(postJson(FN_URL, "http://evil.com"), {}, fns),
		)
	...
	it("CSRF7: port mismatch → 403", async () => {
		...
		const res = await runInContext(() =>
			handleServerFnRequest(postJson(url, "http://localhost:4000"), {}, fns),
		)
		expect(res.status).toBe(403)
	})
	...
	it("CSRF8: scheme mismatch (http vs https) → 403", async () => {
		...
		const url = "https://app.com/_fn/test-fn/test"
		const res = await runInContext(() =>
			handleServerFnRequest(postJson(url, "http://app.com"), {}, fns),
		)
		expect(res.status).toBe(403)
	})
	...
	it("CSRF10: subdomain mismatch → 403", async () => {
		...
		const res = await runInContext(() =>
			handleServerFnRequest(postJson(url, "https://evil.example.com"), {}, fns),
		)
		expect(res.status).toBe(403)
	})
	...
	it("CSRF11: rejects before authentication runs", async () => {
		...
		const res = await runInContext(() =>
			handleServerFnRequest(postJson(FN_URL, "http://evil.com"), {}, fns, authFn),
		)
		expect(res.status).toBe(403)

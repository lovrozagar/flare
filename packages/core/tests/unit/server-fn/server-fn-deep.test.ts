...
describe("concurrent request isolation", () => {
	it("concurrent stream requests produce independent NDJSON", async () => {
		...
		const c1 = msgs1
			.filter((m): m is { c: string } => "c" in (m as Record<string, unknown>))
			.map((m) => m.c)
		const c2 = msgs2
			.filter((m): m is { c: string } => "c" in (m as Record<string, unknown>))
			.map((m) => m.c)
	...
	it("concurrent revalidation calls don't leak tags", async () => {
		...
		const [r1, r2] = await Promise.all([
			runWithServerContext(
				{ nonce: "a", request: new Request("http://localhost"), store: store },
				() => handleServerFnRequest(postReq("/_fn/rev/rev", { tags: ["a"] }), {}, fns),
			),
			runWithServerContext(
				{ nonce: "b", request: new Request("http://localhost"), store: store },
...
describe("streaming + auth combo", () => {
	...
	it("auth passes, UnauthenticatedError mid-stream in NDJSON", async () => {
		...
		const errMsg = messages.find(
			(m): m is { e: { message: string } } => "e" in (m as Record<string, unknown>),
		)

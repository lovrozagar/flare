...
const { buildHandler, dataRequest, makeRequest, parseNDJSON, readResponseBody } = await import("./fixtures");
...
describe("middleware — next", () => {
	...
	it("next continues to route handler — NDJSON path works", async () => {
		...
		const loader = messages.find((m) => m.t === "l" && (m.d as Record<string, unknown>)?.title === "About");

...
function buildRedirectTree(): TreeNode {
	...
	const redirModule = makeRedirectModule("/secret-admin-panel", "_root_/public-page", "_root_/public-page");
...
describe("Redirect response — must not expose internal target URL in error responses", () => {
	...
	it("redirect loop error message does not contain target URLs", () => {
		...
		const errorMsg = "Redirect loop detected after 6 redirects. Check route redirects for circular references.";

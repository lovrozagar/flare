...
export const route = createPage("_root_/seo")
	...
	.head((ctx) => ({
		...
		description: `SEO test on Standard — ${(ctx.loaderData as { title: string }).title}`,
		meta: {
			author: "Standard Team",
		},
		openGraph: {
			description: "OG description for Standard",
			locale: "en_US",
			siteName: "Flare on Standard",
		...
		twitter: {
			card: "summary",
			description: "Twitter desc for Standard",
			title: `Twitter: ${(ctx.loaderData as { title: string }).title}`,
		},
	}))
	.render((props) => (
		<div>
			<h1 data-testid="seo-heading">SEO — Standard</h1>

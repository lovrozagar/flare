...
```ts
(config: { handler?: (ctx: MiddlewareContext) => Promise<void>; interval: number }) => ServerBuilder;
```
...
interface ServerBuilder<TExcluded extends string = never> {
	...
	cache: "cache" extends TExcluded ? never : (config: ServerCacheConfig) => ServerBuilder<TExcluded | "cache">;
	...
	security: "security" extends TExcluded
		? never
		: (config: SecurityConfig | ((ctx: SecurityContext) => SecurityConfig)) => ServerBuilder<TExcluded | "security">;
	serverContext: "serverContext" extends TExcluded
		? never
		: <T>(fn: ServerContextFactory<T>) => ServerBuilder<TExcluded | "serverContext">;
	sitemap: "sitemap" extends TExcluded ? never : (config: SitemapConfig) => ServerBuilder<TExcluded | "sitemap">;
...
interface ClientBuilder<TExcluded extends string = never> {
	onHydrated: "onHydrated" extends TExcluded ? never : (fn: () => void) => ClientBuilder<TExcluded | "onHydrated">;
...
```ts
import { mount } from "@lovrozagar/flare/middleware"

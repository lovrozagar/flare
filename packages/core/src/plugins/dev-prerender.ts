 ...
 */
export function createDevPrerenderPlugin(config: FlarePluginConfig, entries: ResolvedEntries): VitePlugin {
	...
	async function prerenderRoutes(routes: PrerenderRoute[], ssrRunner: SsrEnvironment["runner"]): Promise<void> {
		...
		try {
			...
			if (result.entries.length > 0) {
				process.stderr.write(`[flare:dev-prerender] Pre-rendered ${result.entries.length} SSG route(s)\n`);
			}

...
function localeAllows(paramName: string, value: string, localeMatch?: LocaleMatch): boolean {
	return (
		...
		localeMatch.locales.includes(value)
	)
}
		...
		for (const node of children) {
			...
			if (optionalName) {
				...
				if (
					...
					localeAllows(optionalName, currentSeg, localeMatch)
				) {
...
function readLocaleCfg(
	value: unknown,
): { locales: readonly string[]; paramName?: string } | undefined {
 ...
 */
export function extractLocaleMatchFromModule(

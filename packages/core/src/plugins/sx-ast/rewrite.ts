...
function isJSXExpressionContainer(n: unknown): n is { type: "JSXExpressionContainer"; expression: Expression; start: number; end: number } {
	return typeof n === "object" && n !== null && (n as { type: string }).type === "JSXExpressionContainer"
}

function isCallExpression(n: unknown): n is { type: "CallExpression"; callee: { type: string; name?: string }; arguments: Expression[]; start: number; end: number } {
	return typeof n === "object" && n !== null && (n as { type: string }).type === "CallExpression"
}

function isArrayExpression(n: unknown): n is { type: "ArrayExpression"; elements: Array<Expression | null>; start: number; end: number } {
...
function collectClassLiterals(expr: unknown): string[] {
	...
	const node = expr as { type?: string; left?: unknown; right?: unknown; consequent?: unknown; alternate?: unknown } | null
	...
	if (node.type === "ConditionalExpression") {
		return [
			...collectClassLiterals(node.consequent),
			...collectClassLiterals(node.alternate),
		]
	}
	...
	if (trimmed.startsWith("@")) {
		...
		if (braceIdx !== -1) {
			...
			for (let i = braceIdx; i < trimmed.length; i++) {
				if (trimmed[i] === "{") { if (depth++ === 0) innerStart = i + 1 }
				else if (trimmed[i] === "}") { if (--depth === 0) { innerEnd = i; break } }
			}
			const inner = innerStart !== -1 && innerEnd !== -1
				? trimmed.slice(innerStart, innerEnd).trim()
				: ""
...
function resolveClassAttr(
	...
	if (isJSXExpressionContainer(cv)) {
		...
		if (isCnCall(expr)) {
			...
			return { expr: srcOf(source, expr as { start: number; end: number }), kind: "expr", wasArray: false, wasDynamic: false }
		}
...
export function rewriteModule(source: string, ctx: RewriteCtx): RewriteResult | null {
	...
	function emitClassAttr(classAttr: JSXAttribute | null, insertPos: number, literal: string | null, expr: string | null): void {
		let attrText: string
		if (literal !== null) {
	...
	function handleOpeningElement(opening: JSXOpeningElement): void {
		...
		if (spread && classAttr && dom) {
			...
			/* c8 ignore next 4 -- classExpr always non-null here: spread+classAttr converts literal→expr above */
				...
				if (isObjectExpression(expr)) {
					...
					if (evalResult.ir !== null) {
						...
						if (atomic.vars.length > 0) {
							...
							const insertAt = opening.attributes.length > 0
	...
	function handleCssAttr(
		...
		/* c8 ignore next 4 -- unreachable: JSX attr values are null | Literal | JSXExpressionContainer only */
		else {
			/* c8 ignore next */
			return
		}
	...
	if (neededImports.size > 0) {
		...
		let lastImportEnd = 0
		let existingStylesImport: { start: number; end: number; specifiers: Array<{ local: { name: string }; end: number }> } | null = null
		...
		for (const stmt of prog.body) {
			...
			const s = stmt as { source: { value: string }; start: number; end: number; specifiers: Array<{ local: { name: string }; end: number }> }
			if (s.source.value === "flare/styles") existingStylesImport = s
		}
		...
		if (existingStylesImport) {
			...
			const importLine = `import { ${specifiers} } from "flare/styles"\n`
			if (lastImportEnd > 0) {

...
export function peelInlineTags(html: string): { html: string; tags: string[] } {
	const tags: string[] = [];
	const next = html.replace(/<(script|style)(\s[^>]*)?>([\s\S]*?)<\/\1>/gi, (full, tag: string, attrs = "") => {
		...
		configureServer(server: unknown) {
			...
			return () => {
				...
				vite.middlewares.use(async (req, res, next) => {
					...
					try {
						const mod = ssrEnv.runner ? await ssrEnv.runner.import(`./${entries.server}`) : undefined;
						...
						if (contentType.includes("text/html")) {
							...
							const htmlSetCookies =
								"getSetCookie" in response.headers
									...
									: [];
							response.headers.forEach((value, key) => {
								if (key === "content-type" || key === "content-length" || key === "set-cookie") return;

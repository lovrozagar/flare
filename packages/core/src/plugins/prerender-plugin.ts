...
export function createPrerenderPlugin(config: {
	...
	const prerenderConfig =
		typeof config.prerender === "object" ? config.prerender : ({} as PrerenderPluginConfig)

	return {
		async closeBundle(this: {
			environment?: { config?: { root?: string }; name?: string }
		}): Promise<void> {
			...
			if (sitemapConfig) {
				...
				if (sitemapConfig.robotsTxt !== false && sitemapConfig.robotsTxt !== undefined) {
					...
					const rules =
						typeof sitemapConfig.robotsTxt === "string" ? sitemapConfig.robotsTxt : undefined
					writeFileSync(
						join(staticDir, "robots.txt"),
						generateRobotsTxt(sitemapUrl, rules),
						"utf-8",
					)
				}

				process.stderr.write(
					`[flare:prerender] Generated sitemap with ${sitemapResult.urls.length} URL(s)\n`,
				)
			}
			...
			const staticParams =
				hasDynamic && resolvedHandler.getStaticParams
					? await resolvedHandler.getStaticParams()
					: undefined

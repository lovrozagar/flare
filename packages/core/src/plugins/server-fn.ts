...
export function transformEnvFns(code: string, isSSR: boolean): string {
	...
	while (cursor < code.length) {
		...
		if (match.type === "server-only" || match.type === "client-only") {
			...
			const keepOriginal =
				(match.type === "server-only" && isSSR) || (match.type === "client-only" && !isSSR)
...
export function replaceServerFnConfigs(
	code: string,
	replacer: (configContent: string) => string,
): string {
	let result = ""
	let cursor = 0
...
export function dropDeadImports(code: string, id: string): string {
	...
	interface ImportInfo { bindings: string[]; end: number; start: number }
		...
		configureServer(server: unknown) {
			...
			const watcher = watch(srcDir, { recursive: true }, (_event, filename) => {
				...
				debounceTimer = setTimeout(() => {
					...
					if (
						prev.length !== serverFnFiles.length ||
						prev.some((f, idx) => f !== serverFnFiles[idx])
					) {

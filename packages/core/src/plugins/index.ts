...
export function resolveDevConfig(raw?: DevConfig | boolean): {
	...
	if (raw === false) return { cdnCache: false, dashboard: false, serverTiming: false, staticCache: false };
...
const FLARE_PREFIX = "@lovrozagar/flare/";
...
export function createCssScopePlugin(): VitePlugin {
	return {
		...
		transform(code: string, id: string): { code: string; map: null } | null {
			...
			const importLine =
				'import { registerCSSByName as __flare_registerCSSByName__ } from "@lovrozagar/flare/styles"\n';
...
function createTwDeprecatedPlugin(): VitePlugin {
	return {
		...
		transform(this: { warn: (msg: string) => void }, code: string, id: string): null {
			if (!id.endsWith(".tsx") && !id.endsWith(".jsx")) return null;
			if (!code.includes("tw=")) return null;
			this.warn(`[flare:tw] tw="..." attribute was dropped. Put Tailwind utilities in class="...". (${id})`);
			return null;
		},
	};
}

...
export function ThemeProvider(props: { children: JSX.Element; config?: ThemeConfig }): JSX.Element {
	const cfg: Required<ThemeConfig> = {
		...
		disableTransitionOnChange: props.config?.disableTransitionOnChange ?? DEFAULT_CONFIG.disableTransitionOnChange,
	...
	createEffect(resolvedTheme, (resolved) => {
		...
		if (cfg.disableTransitionOnChange) {
			...
			const script = document.querySelector("script[nonce]") as HTMLElement | null;
			...
			if (document.body) void getComputedStyle(document.body).opacity;

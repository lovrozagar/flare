...
interface ServerFnBuilderTerminal<TAuth, TInput, TOutput> {
	...
	stream<TChunk>(
		fn: (ctx: StreamContext<TAuth, TInput>) => AsyncGenerator<TChunk>,
	): StreamFn<TInput, TChunk>
}

interface ServerFnBuilderAfterAuth<TAuth, TInput, TOutput> extends ServerFnBuilderTerminal<
	TAuth,
	TInput,
	TOutput
> {
...
interface ServerFnBuilder<TAuth, TInput, TOutput> extends ServerFnBuilderAfterAuth<
	TAuth,
	TInput,
	TOutput
> {
	...
	return {
		handler(fn) {
			...
			const serverFn: ServerFn<TInput, TOutput> = async (_input: TInput): Promise<TOutput> => {
				...
				const validated = state.input
					? ((await runValidator(state.input, _input)) as TInput)
					: _input
		...
		stream<TChunk>(
			...
			const streamFn: StreamFn<TInput, TChunk> = (
				...
				const validatedPromise = state.input
					? runValidator(state.input, _input)
						...
						return {
							...
							return(value?: TChunk) {
								return (
									...
									Promise.resolve({ done: true as const, value: undefined as TChunk })
								)
							},
							throw(e?: unknown) {
								return (
...
function createBuilder<TAuth, TInput, TOutput>(
	state: BuilderState,
...
export function formDataToObject(formData: FormData): Record<string, File | string | string[]> {
	...
	for (const [key, val] of formData) {
		if (key === "__flare_fn" || key === "__proto__" || key === "constructor" || key === "prototype")
			continue

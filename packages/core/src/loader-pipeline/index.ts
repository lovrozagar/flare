...
import { getServerContext, serverLog } from "flare/server-context"
...
export async function runPipeline<TEnv = unknown>(
	config: PipelineConfig<TEnv>,
	...
	const matchPromises = internalRoutes.map(async (ir): Promise<PipelineMatch> => {
		...
		const matchId = computeMatchId({
			loaderDeps: ir.route.effectsConfig?.loaderDeps as
				| ((opts: { search: SearchParams }) => unknown[])
				| undefined,
		...
		if (ssrConfig && resolvedStore && cacheKey) {
			try {
				...
				if (cached) {
					const isFresh =
						ssrConfig.staleTime === undefined ||
						Date.now() - cached.storedAt < parseMilliseconds(ssrConfig.staleTime)
					if (isFresh) {
		...
		try {
			...
			if (ssrConfig && resolvedStore && cacheKey) {
				...
				if (ssrConfig.tags) {
					tags =
						typeof ssrConfig.tags === "function"
							? ssrConfig.tags({ params: ir.validatedParams })
							: ssrConfig.tags
				}
				try {
	...
	const routeLocationMap = new Map<
		ResolvedRoute,
		Location<Record<string, string | string[]>, SearchParams, string>
	>()
	for (const ir of internalRoutes) {

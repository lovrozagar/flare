...
type _PathParams<T extends string> = T extends `${infer H}/${infer R}`
	? _Segment<H> & _PathParams<R>
	: _Segment<T>

type FlatPathParams<T extends string> =
	_PathParams<T> extends infer R ? { [K in keyof R]: R[K] } : never

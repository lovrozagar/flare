...
function validateTokenDeclarations(name: string, tokens: Token[]): void {
	...
	while (i < tokens.length && tokens[i].t !== T_EOF) {
		...
		if (tok.t === T_IDENT || (tok.t === T_DELIM && tok.v === "*")) {
			...
			if (
				i < tokens.length &&
				(tokens[i].t === T_SEMI || tokens[i].t === T_RBRACE || tokens[i].t === T_EOF)
			) {
...
>(name: string, config: StylesConfig<S, V>): StylesResult {
	...
	/* build CSS */
	const stateSelectors = config.state
		? createStateSelectors(config.state)
		: ({} as StateSelectors<S>)
	let cssText = ""
	...
	if (config.css) {
		const cssPart =
			typeof config.css === "function" ? config.css(stateSelectors, varAccessors) : config.css
		if (cssText.length > 0) {
			cssText += `\n${cssPart}`
		} else {
			cssText = cssPart

...
export function useRouterContext(): FlareProviderContext {
	...
	if (!ctx)
		throw new Error("useRouterContext() called outside FlareProvider. Ensure <FlareProvider> wraps your app root.");
	return ctx;
}
...
export function useRouter(): FlareRouter {
	...
	return {
		buildLocation: (options) => {
			...
			const result = matchRoute(ctx.routeTree, parsed.pathname, ctx.caseSensitive, toLocaleMatch(ctx.localeConfig));
			return {
				hash: parsed.hash,
...
function ErrorBoundaryWrapper(props: { children: JSX.Element; depth: number; match: ClientMatch }): JSX.Element {
	const ctx = useRouterContext();

...
export type RouterArg =
	| MarkedRouterConfig
	| (() => MarkedRouterConfig | Promise<MarkedRouterConfig>)
...
export async function hydrate(router: RouterArg, options?: HydrateOptions): Promise<void> {
	...
	if (modules) {
		...
		const rootMatchState = rootLayout
			? state.matches.find((m) => m.virtualPath === rootLayout.virtualPath)
			: undefined
		...
		setupNavigation(
			...
			},
			{
				...
				initialRouteIds,
				keepalive: (raw.c.router as Record<string, unknown> | undefined)?.keepalive as
					| false
					| number
					| undefined,
				locale: r.locale,

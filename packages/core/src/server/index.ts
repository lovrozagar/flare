...
export { background } from "flare/server-context"
...
export interface ServerBuilder<TExcluded extends string = never> {
	...
	cache: "cache" extends TExcluded
		? never
		: (config: HandlerCacheConfig) => ServerBuilder<TExcluded | "cache">
	fetch(
		request: Request,
		env?: unknown,
		ctx?: { waitUntil?: (p: Promise<unknown>) => void },
	...
	sitemap: "sitemap" extends TExcluded
		? never
		: (config: SitemapSubmitConfig) => ServerBuilder<TExcluded | "sitemap">
	tracing: "tracing" extends TExcluded
		? never
		: (config: TracingConfig) => ServerBuilder<TExcluded | "tracing">

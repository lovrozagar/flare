...
export function renderHeadToHtml(head: HeadConfig, nonce: string): string {
	...
	if (head.robots) {
		...
		if (r["max-image-preview"] !== undefined)
			directives.push(`max-image-preview:${r["max-image-preview"]}`)
		if (r["max-video-preview"] !== undefined)
			directives.push(`max-video-preview:${r["max-video-preview"]}`)
	...
	if (head.meta) {
		...
		if (m.appleMobileWebAppCapable !== undefined) {
			parts.push(
				`<meta name="apple-mobile-web-app-capable" content="${escapeAttr(m.appleMobileWebAppCapable)}">`,
			)
		}
		...
		if (m.appleMobileWebAppTitle !== undefined) {
			parts.push(
				`<meta name="apple-mobile-web-app-title" content="${escapeAttr(m.appleMobileWebAppTitle)}">`,
			)
		}
	...
	if (head.openGraph) {
		...
		if (og.title !== undefined)
			parts.push(`<meta property="og:title" content="${escapeAttr(og.title)}">`)
		if (og.description !== undefined)
			parts.push(`<meta property="og:description" content="${escapeAttr(og.description)}">`)
		if (og.type !== undefined)
			parts.push(`<meta property="og:type" content="${escapeAttr(og.type)}">`)
		if (og.url !== undefined) parts.push(`<meta property="og:url" content="${escapeAttr(og.url)}">`)
		if (og.siteName !== undefined)
			parts.push(`<meta property="og:site_name" content="${escapeAttr(og.siteName)}">`)
		if (og.locale !== undefined)
			parts.push(`<meta property="og:locale" content="${escapeAttr(og.locale)}">`)
		...
		if (og.videos) {
			for (const video of og.videos) {
				...
				if (video.secureUrl)
					parts.push(
						`<meta property="og:video:secure_url" content="${escapeAttr(video.secureUrl)}">`,
					)
				if (video.type)
					parts.push(`<meta property="og:video:type" content="${escapeAttr(video.type)}">`)
		...
		if (og.audio) {
			for (const audio of og.audio) {
				...
				if (audio.secureUrl)
					parts.push(
						`<meta property="og:audio:secure_url" content="${escapeAttr(audio.secureUrl)}">`,
					)
				if (audio.type)
					parts.push(`<meta property="og:audio:type" content="${escapeAttr(audio.type)}">`)
			}
		}
	}
	...
	if (head.twitter) {
		...
		if (tw.card !== undefined)
			parts.push(`<meta name="twitter:card" content="${escapeAttr(tw.card)}">`)
		if (tw.site !== undefined)
			parts.push(`<meta name="twitter:site" content="${escapeAttr(tw.site)}">`)
		if (tw.creator !== undefined)
			parts.push(`<meta name="twitter:creator" content="${escapeAttr(tw.creator)}">`)
		if (tw.title !== undefined)
			parts.push(`<meta name="twitter:title" content="${escapeAttr(tw.title)}">`)

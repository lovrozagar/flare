...
import {
	FLARE_CACHE_HEADER,
	FLARE_RENDER_HEADER,
	type FlareCacheStatus,
	type FlareRenderMode,
} from "./headers.ts"
...
): Promise<ResolvedQueryClient | undefined> {
...
async function applyResponseHandlers(
	response: Response,
...
async function handleRevalidateRequest<TEnv>(
	...
	const tiers = Array.isArray(body.tiers)
		? body.tiers.filter((t): t is string => typeof t === "string")
		: undefined
	...
	const resolvedStore =
		typeof cacheConfig.store === "function" ? cacheConfig.store(env) : cacheConfig.store
...
async function handleSitemapSubmitRequest<TEnv>(
	...
	const expectedSecret =
		typeof sitemapConfig.secret === "function" ? sitemapConfig.secret(env) : sitemapConfig.secret
	...
	if (engines.google) {
		const credentials =
			typeof engines.google.credentials === "function"
				? engines.google.credentials(env)
				: engines.google.credentials
		try {
	...
	if (engines.bing && urls && urls.length > 0) {
		const apiKey =
			typeof engines.bing.apiKey === "function" ? engines.bing.apiKey(env) : engines.bing.apiKey
		try {
	...
	if (engines.indexNow && urls && urls.length > 0) {
		const key =
			typeof engines.indexNow.key === "function" ? engines.indexNow.key(env) : engines.indexNow.key
		try {
...
>(
	...
	const handler: ServerHandler<TEnv> = {
		async fetch(request: Request, env: TEnv): Promise<Response> {
			...
			let resolvedStore =
				typeof config.cache?.store === "function" ? config.cache.store(env) : config.cache?.store
			...
			if (config.cache?.cdn) {
				resolvedCdnPurge =
					typeof config.cache.cdn === "function" ? config.cache.cdn(env) : config.cache.cdn
			}
				...
				async () => {
					...
					try {
							...
							if (ct.includes("form")) {
								...
								if (typeof flareFnId === "string" && serverFns.has(flareFnId)) {
									...
									if (reg) {
										...
										try {
											const fnResponse = await handleServerFnRequest(
												fnRequest,
												env,
												serverFns,
												serverFnAuthFn,
						...
						if (url.pathname === "/_flare/sitemap/submit" && config.sitemap) {
							/* Internal endpoints bypass middleware response handlers — only security headers */
							let sitemapResponse = await handleSitemapSubmitRequest(
								request,
								url,
								env,
								config.sitemap,
								...
								if (partial) {
									...
									try {
										ssrResult = renderToStream({
											...
											cause: "enter",
											deferContexts:
												pipelineResult.deferContexts.size > 0
													? pipelineResult.deferContexts
													: undefined,
						...
						if (staticMeta && resolvedStore && !isISRBgRequest) {
							...
							if (entry) {
								...
								const isrRevalidate =
									isrRevalidateRaw !== undefined ? parseSeconds(isrRevalidateRaw) : undefined
								const isStale =
									staticMeta.mode === "isr" &&
								...
								if (isStale && !request.headers.get("x-isr-bg") && !isrInFlight.has(storeKey)) {
									...
									const revalidatePromise = (async () => {
										try {
											...
											let storedHtml = html
											const bgNonce =
												extractNonce(html) ?? extractNonce(headers["content-security-policy"] ?? "")
											if (bgNonce) {
											...
											const freshTags = surrogateKey
												? surrogateKey.split(" ").filter(Boolean)
												: entry.tags
						...
						const staleMatchIds = (request.headers.get("x-m")?.split(",") ?? []).slice(
							0,
							MAX_STALE_IDS,
						)
						...
						if (isDataRequest && staleMatchIds.length > 0) {
							for (const route of resolvedRoutes) {
								...
								const isStale = staleMatchIds.some(
									(id) => id === route.virtualPath || id.startsWith(prefix),
								)
								if (!isStale) {
						...
						const logsForTransport =
							collectedLogs && collectedLogs.length > 0 ? collectedLogs : undefined
							...
							if (!isrInFlight.has(isrStoreKey)) {
								...
								const isrPopulate = (async () => {
									try {
										...
										const populateNonce =
											extractNonce(html) ??
											extractNonce(resHeaders["content-security-policy"] ?? "")
										if (populateNonce) {

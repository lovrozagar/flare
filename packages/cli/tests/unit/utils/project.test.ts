...
describe("resolveProject", () => {
	...
	it("detects flare in dependencies", () => {
		writeFileSync(join(tmp, "package.json"), JSON.stringify({ dependencies: { "@lovrozagar/flare": "^0.1.0" } }));
	...
	it("detects flare in devDependencies", () => {
		writeFileSync(join(tmp, "package.json"), JSON.stringify({ devDependencies: { "@lovrozagar/flare": "^0.1.0" } }));
		const result = resolveProject(tmp);
		expect(result.hasFlare).toBe(true);
	});

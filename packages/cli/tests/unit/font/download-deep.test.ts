...
describe("downloadFonts — deep edge cases", () => {
	...
	it("multiple batches each call onProgress", async () => {
		...
		const mockFetch = vi.fn().mockImplementation(() => Promise.resolve(new Response(WOFF2_BYTES, { status: 200 })));
	...
	it("force overwrites file content", async () => {
		...
		const mockFetch = vi.fn().mockImplementation(() => Promise.resolve(new Response(newBytes, { status: 200 })));
	...
	it("deeply nested directories created automatically", async () => {
		const entries: Array<[string, string]> = [["/fonts/some-font/deep/nested/path/latin.woff2", "https://cdn/1"]];

		const mockFetch = vi.fn().mockImplementation(() => Promise.resolve(new Response(WOFF2_BYTES, { status: 200 })));

		await downloadFonts({ entries, fetch: mockFetch, outDir: tempDir });

		expect(existsSync(join(tempDir, "fonts", "some-font", "deep", "nested", "path", "latin.woff2"))).toBe(true);
	});
});

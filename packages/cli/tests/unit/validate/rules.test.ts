...
describe("runRules", () => {
	it("reports missing-root-layout", () => {
		...
		expect(result).toContainEqual(
			expect.objectContaining({ level: "error", rule: "missing-root-layout" }),
		)
	})
	...
	it("reports duplicate-routes from validation errors", () => {
		...
		expect(result).toContainEqual(
			expect.objectContaining({ level: "error", rule: "duplicate-routes" }),
		)
	})
	...
	it("reports auth-no-boundary", () => {
		const defs = [
			makeDef({ type: "root-layout", virtualPath: "_root_" }),
			makeDef({ authenticateMode: true }),
		]
		const result = runRules(defs, [])
		expect(result).toContainEqual(
			expect.objectContaining({ level: "warning", rule: "auth-no-boundary" }),
		)
	})
	...
	it("reports public-no-head", () => {
		const defs = [
			makeDef({ type: "root-layout", virtualPath: "_root_" }),
			makeDef({ authenticateMode: false }),
		]
		const result = runRules(defs, [])
		expect(result).toContainEqual(
			expect.objectContaining({ level: "warning", rule: "public-no-head" }),
		)
	})
	...
	it("reports public-no-cache", () => {
		...
		const result = runRules(defs, [])
		expect(result).toContainEqual(
			expect.objectContaining({ level: "warning", rule: "public-no-cache" }),
		)
	})
	...
	it("reports suggest-prefetch as info", () => {
		...
		const result = runRules(defs, [])
		expect(result).toContainEqual(
			expect.objectContaining({ level: "info", rule: "suggest-prefetch" }),
		)
	})

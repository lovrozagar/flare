...
async function runAdd(paths: string[], method: string, opts?: { isr?: number; ssg?: boolean }): Promise<void> {
	const project = resolveProject();
	if (!project.hasFlare) {
		process.stderr.write("error: flare not found in dependencies\n");
		process.exit(1);
	...
	for (const path of paths) {
		...
		if (result.success) {
			...
			process.stderr.write(`skip: .${result.method}() already exists or chain not found in ${result.filePath}\n`);
		}
	}
}

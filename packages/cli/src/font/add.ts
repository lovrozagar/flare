...
export async function fontAddNonInteractive(options: FontAddOptions): Promise<FontAddResult> {
	const { fetch: fetchFn, force = false, names, onProgress, projectRoot, registry, subsets, urlMap } = options;
...
export function formatImportSnippets(fonts: ResolvedFont[]): string[] {
	return fonts.map((f) => `import { ${f.camelName} } from "@lovrozagar/flare/fonts/${f.slug}"`);
}

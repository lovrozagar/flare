import { ServerFnValidationError } from "@lovrozagar/flare/errors";
...
export const streamCountFn = createServerFn({ method: "post", name: "stream-count" }).stream(async function* () {
	for (let i = 1; i <= 5; i++) {
		await delay(10);
		yield i;
	}
...
export const streamErrorFn = createServerFn({ method: "post", name: "stream-error" }).stream(async function* () {
...
export const slowStreamFn = createServerFn({ method: "post", name: "slow-stream" }).stream(async function* () {
	for (let i = 1; i <= 5; i++) {
		await delay(200);
		yield { chunk: i };
	}

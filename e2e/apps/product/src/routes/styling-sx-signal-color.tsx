...
export const route = createPage("_root_/styling-sx-signal-color").render(() => {
	...
	return (
		<main data-testid="styling-sx-signal-color">
			<button data-testid="cycle-color" onClick={() => setIdx((i) => (i + 1) % COLORS.length)} type="button">
				Cycle
			</button>
			<div data-testid="signal-color-box" sx={{ color: color(), fontSize: "18px", padding: "12px" }}>
				Color: {color()}
			</div>

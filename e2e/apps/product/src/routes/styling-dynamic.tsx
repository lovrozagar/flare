	...
	return (
		<main data-testid="styling-dynamic">
			...
			<button
				...
				onClick={() => setItems((prev) => [...prev, { color: "rgb(255, 165, 0)", id: prev.length + 1 }])}
				type="button"
			>
				Add Item
			</button>
			<button data-testid="remove-item" onClick={() => setItems((prev) => prev.slice(0, -1))} type="button">
				Remove Item
			</button>
			<button
				data-testid="change-color"
				onClick={() =>
					setDynamicColor((prev) => (prev === "rgb(128, 0, 128)" ? "rgb(255, 165, 0)" : "rgb(128, 0, 128)"))
				}
				type="button"
			>
				Change Color

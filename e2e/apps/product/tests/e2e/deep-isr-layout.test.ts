...
test.describe("Cached layout — basic rendering", () => {
	test("layout renders with cached loader data", async ({ page }) => {
		...
		expect(await page.locator("[data-testid=cached-layout-data]").textContent()).toBe(
			"cached-layout",
		)
	})
	...
	test("layout wraps non-cached child page", async ({ page }) => {
		...
		expect(await page.locator("[data-testid=cached-layout-index-page]").textContent()).toBe(
			"cached-layout-index",
		)
	})

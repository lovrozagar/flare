...
test.describe("Stress: mass boxes", () => {
	...
	test("first and last box have different computed colors", async ({ page }) => {
		...
		const color0 = await page.getByTestId("stress-box-0").evaluate((el) => getComputedStyle(el).color);
		const color19 = await page.getByTestId("stress-box-19").evaluate((el) => getComputedStyle(el).color);
...
test.describe("Stress: mixed styles() + css= + class= Tailwind", () => {
	test("scoped styles() works alongside native css= and class= Tailwind", async ({ page }) => {
		...
		/* styles() scoped */
		const scopedColor = await page.getByTestId("stress-mixed-scoped").evaluate((el) => getComputedStyle(el).color);
		expect(scopedColor).toBe("rgb(100, 0, 100)");

		/* native css= */
		const nativeColor = await page.getByTestId("stress-css-native").evaluate((el) => getComputedStyle(el).color);
		expect(nativeColor).toBe("rgb(0, 200, 100)");

		/* class= Tailwind */
		const twWeight = await page.getByTestId("stress-tw-native").evaluate((el) => getComputedStyle(el).fontWeight);
...
test.describe("Stress: SPA round-trip", () => {
	test("mass styles survive navigation", async ({ page }) => {
		...
		const before = await page.getByTestId("stress-box-0").evaluate((el) => getComputedStyle(el).color);

		await navigateSPA(page, "/about");
		await navigateSPA(page, "/styling-stress");

		const after = await page.getByTestId("stress-box-0").evaluate((el) => getComputedStyle(el).color);

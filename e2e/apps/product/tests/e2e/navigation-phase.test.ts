...
test.describe("navigationPhase signal", () => {
	...
	test("navigationPhase transitions through loading → idle on SPA nav", async ({ page }) => {
		...
		/* Navigate via SPA */
		await page.evaluate(() =>
			(window as unknown as { __flareNavigate?: (to: string) => Promise<void> }).__flareNavigate?.(
				"/about",
			),
		)
		...
		await page.waitForFunction(
			() =>
				(
					window as unknown as { __flareNavigationPhase?: () => string }
				).__flareNavigationPhase?.() === "idle",
			null,
			{ timeout: 5000 },
		)

		const phases = await page.evaluate(
			() => (window as unknown as { __phaseLog?: string[] }).__phaseLog,
		)

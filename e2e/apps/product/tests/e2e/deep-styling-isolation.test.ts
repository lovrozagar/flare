...
test.describe("Isolation: nested parent/child/grandchild", () => {
	...
	test("nested colors dont leak between levels", async ({ page }) => {
		...
		const parentColor = await page.getByTestId("nest-parent").evaluate((el) => getComputedStyle(el).color);
		const childColor = await page.getByTestId("nest-child").evaluate((el) => getComputedStyle(el).color);
		const grandColor = await page.getByTestId("nest-grand").evaluate((el) => getComputedStyle(el).color);
...
test.describe("Isolation: same styles() name different CSS", () => {
	test("two components with same name get scoped independently", async ({ page }) => {
		...
		const colorA = await page.getByTestId("same-name-a").evaluate((el) => getComputedStyle(el).color);
		const colorB = await page.getByTestId("same-name-b").evaluate((el) => getComputedStyle(el).color);
...
test.describe("Isolation: dynamic mutation on one sibling", () => {
	test("toggling highlight on sib-5 doesnt affect others", async ({ page }) => {
		...
		for (let i = 1; i <= 4; i++) {
			initialColors[`sib-${i}`] = await page.getByTestId(`sib-${i}`).evaluate((el) => getComputedStyle(el).color);
		}
...
test.describe("Isolation: post-hydration no duplication", () => {
	test("scoped style tag count at most 1", async ({ page }) => {
		await loadPage(page, "/styling-isolation");
		const count = await page.evaluate(() => document.querySelectorAll("style#__FLARE_SCOPED__").length);
...
test.describe("Isolation: SPA round-trip", () => {
	test("nav away and back preserves all sibling styles", async ({ page }) => {
		...
		for (let i = 1; i <= 5; i++) {
			before[`sib-${i}`] = await page.getByTestId(`sib-${i}`).evaluate((el) => getComputedStyle(el).color);
		}

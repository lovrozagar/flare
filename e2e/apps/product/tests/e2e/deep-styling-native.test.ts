...
test.describe("Native props: SPA navigation", () => {
	test("SPA nav preserves data-c and computed styles for css=", async ({ page }) => {
		...
		const color = await page.getByTestId("css-native-box").evaluate((el) => getComputedStyle(el).color);
	...
	test("SPA nav preserves Tailwind class= styles", async ({ page }) => {
		...
		const display = await page.getByTestId("tw-native-flex").evaluate((el) => getComputedStyle(el).display);
		expect(display).toBe("flex");
	});
});

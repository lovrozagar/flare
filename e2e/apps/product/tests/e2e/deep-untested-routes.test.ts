...
test.describe("Route coverage — styling-nav-stress", () => {
	...
	test("scoped styles apply correct colors", async ({ page }) => {
		...
		const headerColor = await page.locator("[data-testid=stress-header]").evaluate((el) => getComputedStyle(el).color);
		expect(headerColor).toBe("rgb(255, 0, 0)");

		const bodyColor = await page.locator("[data-testid=stress-body]").evaluate((el) => getComputedStyle(el).color);
		expect(bodyColor).toBe("rgb(0, 128, 0)");

		const footerColor = await page.locator("[data-testid=stress-footer]").evaluate((el) => getComputedStyle(el).color);

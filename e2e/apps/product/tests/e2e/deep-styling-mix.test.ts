...
test.describe("Styling mix: isolation", () => {
	test("all styled elements use distinct styling approaches", async ({ page }) => {
		...
		const cssColor = await cssNative.evaluate((el) => getComputedStyle(el).color);
		expect(cssColor).not.toBe("rgb(0, 0, 0)");
...
test.describe("Styling mix: SPA round-trip", () => {
	test("all sources survive SPA navigation", async ({ page }) => {
		...
		const border = await page.getByTestId("mix-head").evaluate((el) => getComputedStyle(el).borderColor);
		expect(border).toBe("rgb(128, 0, 128)");

		const color = await page.getByTestId("mix-scoped").evaluate((el) => getComputedStyle(el).color);
		expect(color).toBe("rgb(0, 0, 255)");

		const fontStyle = await page.getByTestId("mix-css-native").evaluate((el) => getComputedStyle(el).fontStyle);

...
test.describe("sx + Base UI Solid: Dialog trigger", () => {
	...
	test("trigger has sx-applied padding", async ({ page }) => {
		await loadPage(page, "/styling-sx-base-ui");
		const style = await page.getByTestId("bui-trigger").evaluate((el) => {
...
test.describe("sx + Base UI Solid: Dialog.Popup sx styles", () => {
	test("popup has sx-applied white background", async ({ page }) => {
		await loadPage(page, "/styling-sx-base-ui");
		await page.getByTestId("bui-trigger").click();
		const bg = await page.getByTestId("bui-popup").evaluate((el) => getComputedStyle(el).backgroundColor);
	...
	test("popup has sx-applied padding 24px", async ({ page }) => {
		await loadPage(page, "/styling-sx-base-ui");
		await page.getByTestId("bui-trigger").click();
		const padding = await page.getByTestId("bui-popup").evaluate((el) => getComputedStyle(el).padding);
	...
	test("popup has sx-applied border-radius 8px", async ({ page }) => {
		await loadPage(page, "/styling-sx-base-ui");
		await page.getByTestId("bui-trigger").click();
		const br = await page.getByTestId("bui-popup").evaluate((el) => getComputedStyle(el).borderRadius);
		expect(br).toBe("8px");
	});
});
...
test.describe("sx + Base UI Solid: Dialog.Backdrop sx styles", () => {
	test("backdrop has sx semi-transparent background", async ({ page }) => {
		await loadPage(page, "/styling-sx-base-ui");
		await page.getByTestId("bui-trigger").click();
		const bg = await page.getByTestId("bui-backdrop").evaluate((el) => getComputedStyle(el).backgroundColor);
		/* rgba(0,0,0,0.5) */
	...
	test("backdrop has sx position fixed", async ({ page }) => {
		await loadPage(page, "/styling-sx-base-ui");
		await page.getByTestId("bui-trigger").click();
		const pos = await page.getByTestId("bui-backdrop").evaluate((el) => getComputedStyle(el).position);
...
test.describe("sx + Base UI Solid: render prop polymorphism", () => {
	test("render fn: trigger renders as anchor element", async ({ page }) => {
		await loadPage(page, "/styling-sx-base-ui");
		const tag = await page.getByTestId("bui-trigger-link").evaluate((el) => el.tagName.toLowerCase());
		expect(tag).toBe("a");
	});

	test("render fn: anchor has sx-applied underline", async ({ page }) => {
		await loadPage(page, "/styling-sx-base-ui");
		const style = await page.getByTestId("bui-trigger-link").evaluate((el) => getComputedStyle(el).textDecorationLine);

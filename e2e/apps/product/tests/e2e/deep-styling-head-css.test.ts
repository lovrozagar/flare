...
test.describe("Head CSS: SPA nav add", () => {
	test("navigating to head.css page adds stylesheet link", async ({ page }) => {
		...
		const beforeCount = await page.evaluate(() => document.querySelectorAll('link[href="/test-styles.css"]').length);
		expect(beforeCount).toBe(0);

		await navigateSPA(page, "/styling-head-css");
		const afterCount = await page.evaluate(() => document.querySelectorAll('link[href="/test-styles.css"]').length);
...
test.describe("Head CSS: SPA nav remove", () => {
	test("navigating away from head.css page removes stylesheet link", async ({ page }) => {
		await loadPage(page, "/styling-head-css");
		const linkCount = await page.evaluate(() => document.querySelectorAll('link[href="/test-styles.css"]').length);

...
test.describe("Prefetch + Deferred optimization", () => {
	...
	test("SPA navigate to deferred route (no prefetch) → shell + deferred render", async ({ page }) => {
		await loadPage(page, "/");
		await setNavMarker(page);
...
test.describe("Prefetch cache: deferred forces refetch, non-deferred uses cache", () => {
	test("deferred route: prefetch intent → navigate fires second NDJSON request", async ({ page }) => {
		const cap = setupConsoleCapture(page);
		await loadPage(page, "/");
	...
	test("non-deferred route: prefetch intent → navigate does NOT fire second request", async ({ page }) => {
		const cap = setupConsoleCapture(page);
		await loadPage(page, "/");
...
test.describe("SSR deferred streaming", () => {
	test("SSR HTML contains deferred marker in FlareState and streaming resolver", async ({ page }) => {

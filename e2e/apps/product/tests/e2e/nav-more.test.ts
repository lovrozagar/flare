...
test.describe("disabled toggle", () => {
	test("starts as span then becomes anchor", async ({ page }) => {
		...
		expect(await page.getByTestId("toggle-link").evaluate((n) => n.tagName.toLowerCase())).not.toBe("a");
...
test.describe("download + concurrent + phase", () => {
	...
	test("navigation phase is a string", async ({ page }) => {
		...
		const phase = await page.evaluate(() => {
			const fn = (window as unknown as { __flareNavigationPhase?: () => string }).__flareNavigationPhase;
			return fn?.();
		});

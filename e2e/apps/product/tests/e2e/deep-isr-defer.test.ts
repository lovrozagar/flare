...
test.describe("ISR + Defer — first SSR miss with streaming", () => {
	test("single defer renders pending then resolves", async ({ page }) => {
		...
		expect(await page.locator("[data-testid=isr-defer-resolved]").textContent()).toBe("deferred-comment");
	});
	...
	test("multi defer — both fast and slow resolve", async ({ page }) => {
		...
		expect(await page.locator("[data-testid=isr-multi-fast-resolved]").textContent()).toBe("fast-result");

		/* Slow resolves after (~300ms) */
		await expect(page.locator("[data-testid=isr-multi-slow-resolved]")).toBeVisible({
			timeout: 5000,
		});
		expect(await page.locator("[data-testid=isr-multi-slow-resolved]").textContent()).toBe("slow-result");
	});

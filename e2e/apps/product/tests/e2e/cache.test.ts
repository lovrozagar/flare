...
test.describe("KV / ISR / duration / Vary", () => {
	...
	test("ISR ETag is weak when present; If-None-Match still 200 with matching nonce", async ({ request }) => {
		await request.get("/isr-test");
...
test.describe("SSG / ISR param allowlist", () => {
	...
	test("listed ISR slugs HIT from prerender artifacts @prod-only @node-only", async ({ request }) => {

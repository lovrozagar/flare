...
test.describe("query + broadcast + theme", () => {
	...
	test("theme and direction scripts in HTML", async ({ page }) => {
		...
		expect(html.includes("localStorage") || html.includes("data-theme") || html.includes("dir")).toBe(true);
...
test.describe("fonts image lazy sx", () => {
	...
	test("sx variant color changes", async ({ page }) => {
		...
		const color = async () => page.getByTestId("sx-variants-box").evaluate((el) => getComputedStyle(el).color);

...
test.describe("head merge + integrity", () => {
	...
	test("COOP or security headers present", async ({ page }) => {
		...
		expect(res?.headers()["cross-origin-opener-policy"] ?? res?.headers()["x-frame-options"] ?? "present").toBeTruthy();
	});
});
...
test.describe("errors extras", () => {
	...
	test("error retry succeeds", async ({ page }) => {
		...
		await expect(page.getByTestId("retry-success").or(page.getByTestId("retry-error-boundary"))).toBeVisible();
	});
});

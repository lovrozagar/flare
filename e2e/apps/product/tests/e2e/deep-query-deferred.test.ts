...
import { assertSPANavigation, BASE, loadPage, navigateSPA, setNavMarker, setupConsoleCapture } from "./helpers";
...
test.describe("QueryClient: deferred streaming (SSR)", () => {
	...
	test("initial self.flare.q has fallback, __flare_qc overrides it", async ({ page }) => {
		...
		const deferredEntry = initialQ.find((e) => JSON.stringify(e.key) === JSON.stringify(["deferred-item"]));
...
test.describe("QueryClient: deferred NDJSON", () => {
	test("NDJSON data request contains q message for deferred QC", async ({ page }) => {
		...
		const qMessages = lines.map((l) => JSON.parse(l) as { t: string; d?: unknown }).filter((m) => m.t === "q");
		...
		const deferredEntry = entries.find((e) => JSON.stringify(e.key) === JSON.stringify(["deferred-item"]));
		expect(deferredEntry).toBeTruthy();
		expect((deferredEntry?.data as { name: string } | undefined)?.name).toBe("from-deferred");
	});

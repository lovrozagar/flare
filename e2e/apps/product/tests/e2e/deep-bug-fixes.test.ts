...
test.describe("Bug 14: Revalidation endpoint — malformed JSON returns JSON 400", () => {
	test("malformed JSON body returns 400 with JSON content-type (not HTML 500)", async ({
		request,
	}) => {
...
test.describe("Bug 28: head.meta fields handled during SPA navigation", () => {
	test("SSR renders author and creator meta tags", async ({ page }) => {
		...
		await expect(page.locator('head meta[name="creator"]')).toHaveAttribute(
			"content",
			"Flare Framework",
		)
...
test.describe("Bug 29: custom.meta handled during SPA navigation", () => {
	test("SSR renders custom meta tag", async ({ page }) => {
		...
		await expect(page.locator('head meta[name="custom-verify"]')).toHaveAttribute(
			"content",
			"flare-e2e-verify",
		)
...
test.describe("Bug 30: sized favicons handled during SPA navigation", () => {
	test("SSR renders sized favicon link tags", async ({ page }) => {
		...
		await expect(page.locator('head link[rel="icon"][sizes="192x192"]')).toHaveAttribute(
			"href",
			"/icons/icon-192.png",
		)
		await expect(page.locator('head link[rel="icon"][sizes="512x512"]')).toHaveAttribute(
			"href",
			"/icons/icon-512.png",
		)

...
test.describe("Interactive styles: SPA round-trip", () => {
	test("toggled state resets after SPA nav away and back", async ({ page }) => {
		...
		const activeColor = await page.getByTestId("active-box").evaluate((el) => getComputedStyle(el).color);
		expect(activeColor).toBe("rgb(0, 128, 0)");
		...
		const resetColor = await page.getByTestId("active-box").evaluate((el) => getComputedStyle(el).color);

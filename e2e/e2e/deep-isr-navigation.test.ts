...
test.describe("ISR — NDJSON data requests on SPA navigation", () => {
	test("SPA nav to ISR route fetches NDJSON data", async ({ page, request }) => {
		...
		const ndjsonPromise = page.waitForResponse(
			(response) =>
				response.url().includes("/isr-test") &&
				(response.headers()["content-type"]?.includes("ndjson") ?? false),
			{ timeout: 10000 },
		)
	...
	test("SPA nav to ISR defer route fetches data with deferred values", async ({
		page,
		request,
	}) => {
		/* Prime ISR store */

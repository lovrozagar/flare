...
test.describe("CSP and nonce verification", () => {
	test("NDJSON CSP includes script-src with nonce", async ({ request }) => {
		const response = await request.get("/about", {
			headers: { "x-d": "1" },
		})
	...
	test("NDJSON CSP nonce differs between requests", async ({ request }) => {
	...
	test("error NDJSON response includes CSP header", async ({ request }) => {
...
test.describe("Cache-Control from CDN config", () => {
	...
	test("page content renders with CDN cache config", async ({ page }) => {
		...
		await expect(page.locator("[data-testid=cache-message]")).toHaveText(
			"Page with CDN cache config",
		)
		cap.assertClean()
	})
})

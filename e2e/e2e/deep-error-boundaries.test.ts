 ...
 */
async function spaNavToErrorRoute(
...
test.describe("SSR error type boundaries", () => {
	...
	test("/throw-unauthenticated returns 401 and renders unauthenticated boundary", async ({
		page,
	}) => {
		...
		await expect(page.locator("[data-testid=unauthenticated-boundary]")).toContainText(
			"Please log in",
		)
	})
	...
	test("/error-string returns 500 and renders error boundary for thrown string", async ({
		page,
	}) => {
		...
		await expect(page.locator("[data-testid=error-string-message]")).toContainText(
			"plain string error",
		)
	})

	test("error-string boundary receives wrapped Error (pipeline normalizes thrown strings)", async ({
		page,
	}) => {
...
test.describe("Error boundary walk-up", () => {
	...
	test("/layout-catches-child/broken-child stops at layout errorRender (not root)", async ({
		page,
	}) => {
...
test.describe("NDJSON error responses", () => {
	test("NDJSON /broken returns error message (t:e) with correct matchId and message", async ({
		page,
	}) => {
	...
	test("NDJSON /error-string returns error message for thrown string", async ({ page }) => {
		...
		const hasStringRef =
			JSON.stringify(errLine).includes("plain string error") || body.includes("plain string error")

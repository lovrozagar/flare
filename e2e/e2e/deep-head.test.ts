...
test.describe("Deep: no stale/duplicate meta tags after round-trip", () => {
	...
	test("about → head-demo → about: about description replaces head-demo description", async ({
		page,
	}) => {
		const cap = setupConsoleCapture(page)
...
test.describe("Deep: head updates through multi-hop SPA chain", () => {
	test("title tracks through / → /about → /head-demo → /blog → /blog/hello-world", async ({
		page,
	}) => {
		const cap = setupConsoleCapture(page)
		await loadPage(page, "/")
...
test.describe("Deep: title fallback to root layout when page has no .head()", () => {
	test("navigating from Home to page without .head() resets title to root layout", async ({
		page,
	}) => {
		const cap = setupConsoleCapture(page)
		await loadPage(page, "/")

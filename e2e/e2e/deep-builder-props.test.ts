...
test.describe("Deep: nested layout+page preloaderContext independence", () => {
	...
	test("layout has its own preloaderContext (not page's)", async ({ page }) => {
		...
		const layoutTs = Number(
			await page.locator("[data-testid=layout-preloader-timestamp]").textContent(),
		)
		expect(layoutTs).toBeGreaterThan(0)

		/* Page preloader returns pageTimestamp */
		const pageTs = Number(
			await page.locator("[data-testid=page-preloader-timestamp]").textContent(),
		)
		expect(pageTs).toBeGreaterThan(0)

		/* Both timestamps exist independently — layout doesn't see page's timestamp */
		expect(await page.locator("[data-testid=layout-preloader-pathname]").textContent()).toBe(
			"/props-nested",
		)
		expect(await page.locator("[data-testid=page-preloader-pathname]").textContent()).toBe(
			"/props-nested",
		)
	})
	...
	test("layout preloaderContext timestamp matches layout loader received value", async ({
		page,
	}) => {
		await loadPage(page, "/props-nested")
	...
	test("layout location matches page location", async ({ page }) => {
		await loadPage(page, "/props-nested")
		expect(await page.locator("[data-testid=layout-location-pathname]").textContent()).toBe(
			"/props-nested",
		)
		expect(await page.locator("[data-testid=page-location-pathname]").textContent()).toBe(
			"/props-nested",
		)
	})
})

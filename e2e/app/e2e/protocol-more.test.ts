...
test.describe("NDJSON protocol deep", () => {
	...
	test("SSR about text matches after hydrate", async ({ page }) => {
		...
		expect(html).toContain("About the current Flare consumer.")
		await loadPage(page, "/about")
		await expect(page.getByTestId("about-content")).toHaveText("About the current Flare consumer.")
	})
	...
	test("multi defer resolves both", async ({ page }) => {
		...
		await expect(page.getByTestId("fast-value")).toHaveText("fast")
		await expect(page.getByTestId("slow-value")).toHaveText("slow")
	})

	test("deferred error surface", async ({ page }) => {
		await page.goto("/deferred-error")
		await expect(page.getByTestId("deferred-error-msg")).toContainText("deferred failed")
	})
})

			...
			"next-router-state-tree": encodeURIComponent(
				JSON.stringify([
					"",
					{
						children: [
							"posts",
							{ children: [["slug", POST_SLUG, "d"], { children: ["__PAGE__", {}] }] },
						],
					},
				]),
			),
 ...
 */
async function captureSpaHttp(fw: {
	name: string
	port: number
...
async function captureFramework(
	...
	console.log(
		`[${fw.name}] Gzip: SSR=${result.gzipSizes.ssrHtml}B, SPA=${result.gzipSizes.spaPayload ?? "N/A"}B`,
	)
	...
	console.log(
		`[${fw.name}] Initial JS: ${result.initialJsCost.fileCount} files, ${result.initialJsCost.totalBytes}B`,
	)
	...
	console.log(`[${fw.name}] SPA nav JS cost (fresh context)`)
	{
		...
		const jsHandler = async (response: PwResponse) => {
			...
			const ct = response.headers()["content-type"] ?? ""
			const isJs =
				ct.includes("javascript") || ct.includes("module") || /\.(js|mjs|tsx?)(\?|$)/.test(url)
	...
	result.headAfterNav.ogDescription = await page.getAttribute(

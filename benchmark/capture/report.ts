...
function generateReport(results: FrameworkResult[]): string {
	...
	lines.push(
		"| SSR HTML size | " + results.map((r) => formatBytes(r.ssrHtml.size)).join(" | ") + " |",
	)
	lines.push(
		"| HTML only (no state) | " +
			results
				.map((r) => (r.ssrHtml.htmlOnly > 0 ? formatBytes(r.ssrHtml.htmlOnly) : "N/A"))
				.join(" | ") +
			" |",
	)
	lines.push(
		"| Embedded state size | " +
			results
				.map((r) => (r.ssrHtml.stateSize > 0 ? formatBytes(r.ssrHtml.stateSize) : "N/A"))
				.join(" | ") +
			" |",
	)
	...
	if (results.some((r) => r.ttfb)) {
		lines.push(
			"| TTFB SSR (median 5 runs) | " +
				results.map((r) => (r.ttfb ? `${r.ttfb.ssrMs}ms` : "N/A")).join(" | ") +
				" |",
		)
		lines.push(
	...
	lines.push(
		"| Initial page JS (runtime) | " +
			results
				.map((r) => {
					if (r.initialJsCost)
						return `${formatBytes(r.initialJsCost.totalBytes)} (${r.initialJsCost.fileCount} files)`
					return "N/A"
				})
				.join(" | ") +
			" |",
	...
	lines.push(
		"| JS loaded on SPA nav | " +
			results
				.map((r) => {
					if (r.spaJsCost)
						return `${formatBytes(r.spaJsCost.totalBytes)} (${r.spaJsCost.fileCount} files)`
					return "none"
				})
				.join(" | ") +
			" |",
	...
	if (results.some((r) => r.buildOutput)) {
		lines.push(
			"| Client bundle total | " +
				results
					.map((r) => (r.buildOutput ? `${r.buildOutput.totalClientKB} KB` : "N/A"))
					.join(" | ") +
				" |",
		)
		lines.push(
			"| Server bundle total | " +
				results
					.map((r) => (r.buildOutput ? `${r.buildOutput.totalServerKB} KB` : "N/A"))
					.join(" | ") +
				" |",
		)
		lines.push(
			"| Entry chunk size | " +
				results
					.map((r) => (r.buildOutput ? `${r.buildOutput.entryChunkKB} KB` : "N/A"))
					.join(" | ") +
				" |",
		)
	}
	...
	lines.push(
		"| og:title after nav | " +
			results
				.map((r) => (r.headAfterNav.ogTitle ? `"${r.headAfterNav.ogTitle}"` : "missing"))
				.join(" | ") +
			" |",
	)
	lines.push(
	...
	lines.push(
		"| Re-fetches on back/fwd | " +
			results
				.map((r) =>
					r.backForwardRefetches > 0 ? `${r.backForwardRefetches} requests` : "0 (cached)",
				)
				.join(" | ") +
			" |",
	)
	lines.push(
		"| Prefetch on hover | " +
			results
				.map((r) => (r.prefetchRequests > 0 ? `${r.prefetchRequests} requests` : "none"))
				.join(" | ") +
			" |",
	)
	lines.push(
	...
	for (const result of results) {
		...
		if (result.spaNavigation) {
			...
			if (result.name === "TanStack Start") {
				lines.push(
					"*No data request -- loaders are isomorphic and run client-side during SPA navigation.*",
				)
				lines.push(
					"Only `createServerFn()` calls would trigger server RPCs (to `/_serverFn/<id>`).",
				)
				if (result.spaJsCost) {
					lines.push(
		...
		if (result.interactivity) {
			...
			if (result.interactivity.buttonFound) {
				...
				lines.push(
					`- Hydration + event handler: ${result.interactivity.worked ? "**working**" : "**BROKEN**"}`,
				)
			} else {
		...
		if (result.buildOutput) {
			...
			lines.push(
				`- Client bundle: ${result.buildOutput.totalClientKB} KB (${result.buildOutput.chunkCount} files)`,
			)
	...
	if (flare?.spaNavigation) {
		...
		if (flare.wireAnnotation) {
			...
			for (const [type, count] of Object.entries(flare.wireAnnotation.linesByType).sort(
				([, a], [, b]) => b - a,
			)) {
	...
	if (next?.spaNavigation) {
		...
		if (next.wireAnnotation) {
			...
			for (const [type, count] of Object.entries(next.wireAnnotation.linesByType).sort(
				([, a], [, b]) => b - a,
			)) {
				lines.push(`| ${type} | ${count} | ${typeDescs[type] ?? ""} |`)
			}
	...
	lines.push("```json")
	if (flare?.spaNavigation) {
		const loaderLine = flare.spaNavigation.body
			.split("\n")
	...
	lines.push("```")
	if (next?.spaNavigation) {
		const metadataLine = next.spaNavigation.body
			.split("\n")
	...
	if (overheadResults.length > 0) {
		...
		lines.push(
			"| Wire lines | " +
				overheadResults.map((r) => String(r.wireAnnotation?.totalLines ?? 0)).join(" | ") +
				" |",
		)
		lines.push("")
	}
		...
		lines.push(
			"| Total (client + server) | " +
				results
					.map((r) =>
						r.buildOutput
							? `${(r.buildOutput.totalClientKB + r.buildOutput.totalServerKB).toFixed(1)} KB`
							: "N/A",
					)
					.join(" | ") +
				" |",
...
function main(): void {
	...
	console.log(
		`Report written to RESULTS.md (${results.length} frameworks, ${results[0]?.mode ?? "dev"} mode)`,
	)
}

...
test.describe("fresh browser context per locale", () => {
	test("/fr in fresh context → cookie=fr (not polluted by other tests)", async ({ browser }) => {
		const ctx = await browser.newContext()
		const page = await ctx.newPage()
		await page.goto("http://localhost:5001/fr", { waitUntil: "networkidle" })
		await page.waitForTimeout(2000)
	...
	test("/hr in fresh context → cookie=hr", async ({ browser }) => {
		const ctx = await browser.newContext()
		const page = await ctx.newPage()
		await page.goto("http://localhost:5001/hr", { waitUntil: "networkidle" })
		await page.waitForTimeout(2000)
		const cookies = await ctx.cookies()
		expect(cookies.find((c) => c.name === "flare.locale")?.value).toBe("hr")
		await ctx.close()
	...
	test("/ in fresh context → cookie=en", async ({ browser }) => {
		const ctx = await browser.newContext()
		const page = await ctx.newPage()

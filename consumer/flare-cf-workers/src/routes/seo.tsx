...
export const route = createPage("_root_/seo")
	...
	.head((ctx) => ({
		...
		description: `SEO test on CF Workers — ${(ctx.loaderData as { title: string }).title}`,
		meta: {
			author: "CF Workers Team",
		},
		openGraph: {
			description: "OG description for CF Workers",
			locale: "en_US",
			siteName: "Flare on CF Workers",
		...
		twitter: {
			card: "summary",
			description: "Twitter desc for CF Workers",
			title: `Twitter: ${(ctx.loaderData as { title: string }).title}`,
		},
	}))
	.render((props) => (
		<div>
			<h1 data-testid="seo-heading">SEO — CF Workers</h1>

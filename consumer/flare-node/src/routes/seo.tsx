...
export const route = createPage("_root_/seo")
	...
	.head((ctx) => ({
		...
		description: `SEO test on Node — ${(ctx.loaderData as { title: string }).title}`,
		meta: {
			author: "Node Team",
		},
		openGraph: {
			description: "OG description for Node",
			locale: "en_US",
			siteName: "Flare on Node",
		...
		twitter: {
			card: "summary",
			description: "Twitter desc for Node",
			title: `Twitter: ${(ctx.loaderData as { title: string }).title}`,
		},
	}))
	.render((props) => (
		<div>
			<h1 data-testid="seo-heading">SEO — Node</h1>

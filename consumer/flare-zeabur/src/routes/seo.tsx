...
export const route = createPage("_root_/seo")
	...
	.head((ctx) => ({
		...
		description: `SEO test on Zeabur — ${(ctx.loaderData as { title: string }).title}`,
		meta: {
			author: "Zeabur Team",
		},
		openGraph: {
			description: "OG description for Zeabur",
			locale: "en_US",
			siteName: "Flare on Zeabur",
		...
		twitter: {
			card: "summary",
			description: "Twitter desc for Zeabur",
			title: `Twitter: ${(ctx.loaderData as { title: string }).title}`,
		},
	}))
	.render((props) => (
		<div>
			<h1 data-testid="seo-heading">SEO — Zeabur</h1>

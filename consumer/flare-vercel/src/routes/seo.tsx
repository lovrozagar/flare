...
export const route = createPage("_root_/seo")
	...
	.head((ctx) => ({
		...
		description: `SEO test on Vercel — ${(ctx.loaderData as { title: string }).title}`,
		meta: {
			author: "Vercel Team",
		},
		openGraph: {
			description: "OG description for Vercel",
			locale: "en_US",
			siteName: "Flare on Vercel",
		...
		twitter: {
			card: "summary",
			description: "Twitter desc for Vercel",
			title: `Twitter: ${(ctx.loaderData as { title: string }).title}`,
		},
	}))
	.render((props) => (
		<div>
			<h1 data-testid="seo-heading">SEO — Vercel</h1>

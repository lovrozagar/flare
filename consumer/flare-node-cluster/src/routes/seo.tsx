...
export const route = createPage("_root_/seo")
	...
	.head((ctx) => ({
		...
		description: `SEO test on Node Cluster — ${(ctx.loaderData as { title: string }).title}`,
		meta: {
			author: "Node Cluster Team",
		},
		openGraph: {
			description: "OG description for Node Cluster",
			locale: "en_US",
			siteName: "Flare on Node Cluster",
		...
		twitter: {
			card: "summary",
			description: "Twitter desc for Node Cluster",
			title: `Twitter: ${(ctx.loaderData as { title: string }).title}`,
		},
	}))
	.render((props) => (
		<div>
			<h1 data-testid="seo-heading">SEO — Node Cluster</h1>

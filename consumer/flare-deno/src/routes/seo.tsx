...
export const route = createPage("_root_/seo")
	...
	.head((ctx) => ({
		...
		description: `SEO test on Deno — ${(ctx.loaderData as { title: string }).title}`,
		meta: {
			author: "Deno Team",
		},
		openGraph: {
			description: "OG description for Deno",
			locale: "en_US",
			siteName: "Flare on Deno",
		...
		twitter: {
			card: "summary",
			description: "Twitter desc for Deno",
			title: `Twitter: ${(ctx.loaderData as { title: string }).title}`,
		},
	}))
	.render((props) => (
		<div>
			<h1 data-testid="seo-heading">SEO — Deno</h1>

...
export const route = createPage("_root_/seo")
	...
	.head((ctx) => ({
		...
		description: `SEO test on AWS Lambda — ${(ctx.loaderData as { title: string }).title}`,
		meta: {
			author: "AWS Lambda Team",
		},
		openGraph: {
			description: "OG description for AWS Lambda",
			locale: "en_US",
			siteName: "Flare on AWS Lambda",
		...
		twitter: {
			card: "summary",
			description: "Twitter desc for AWS Lambda",
			title: `Twitter: ${(ctx.loaderData as { title: string }).title}`,
		},
	}))
	.render((props) => (
		<div>
			<h1 data-testid="seo-heading">SEO — AWS Lambda</h1>

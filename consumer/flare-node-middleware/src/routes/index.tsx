...
export const route = createPage("_root_/")
	...
	.head(({ loaderData }) => ({
		title: `Flare on Node Middleware — ${(loaderData as { requestId: string }).requestId.slice(0, 8)}`,
	}))
	.headers(({ loaderData }) => ({
		"set-cookie": `flare-visit=${(loaderData as { visitToken: string }).visitToken}; Path=/; SameSite=Lax`,
		"x-deploy-target": "Node Middleware",
		"x-powered-by": "flare",
	}))
	.render((props) => (
		<div>
			<h1>Hello from Flare on Node Middleware</h1>
			<nav>

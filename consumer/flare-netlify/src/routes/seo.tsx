...
export const route = createPage("_root_/seo")
	...
	.head((ctx) => ({
		...
		description: `SEO test on Netlify — ${(ctx.loaderData as { title: string }).title}`,
		meta: {
			author: "Netlify Team",
		},
		openGraph: {
			description: "OG description for Netlify",
			locale: "en_US",
			siteName: "Flare on Netlify",
		...
		twitter: {
			card: "summary",
			description: "Twitter desc for Netlify",
			title: `Twitter: ${(ctx.loaderData as { title: string }).title}`,
		},
	}))
	.render((props) => (
		<div>
			<h1 data-testid="seo-heading">SEO — Netlify</h1>

...
test.describe("Title resolution across navigations", () => {
	...
	test("5-hop title chain: home → head-full → empty → about → head-demo → null-loader", async ({
		page,
	}) => {
		const cap = setupConsoleCapture(page)
		await loadPage(page, "/")
...
test.describe("OG image handling across navigations", () => {
	test("multiple OG images rendered on SSR", async ({ page }) => {
		...
		await expect(page.locator("head meta[property='og:image:width']").nth(0)).toHaveAttribute(
			"content",
			"800",
		)
		await expect(page.locator("head meta[property='og:image:height']").nth(0)).toHaveAttribute(
			"content",
			"400",
		)
	})
	...
	test("OG images replaced when navigating between pages", async ({ page }) => {
		...
		await expect(page.locator("head meta[property='og:image:alt']").nth(0)).toHaveAttribute(
			"content",
			"Top Level",
		)
...
test.describe("Robots meta resolution", () => {
	...
	test("robots meta changes between pages", async ({ page }) => {
		...
		await loadPage(page, "/head-minimal")
		const minimalRobots =
			(await page.locator("head meta[name='robots']").getAttribute("content")) ?? ""
...
test.describe("Keywords meta resolution", () => {
	test("keywords rendered on SSR", async ({ page }) => {
		await loadPage(page, "/head-full")
		await expect(page.locator("head meta[name='keywords']")).toHaveAttribute(
			"content",
			"test, head, seo, flare",
		)
	})
...
test.describe("JSON-LD resolution", () => {
	...
	test("JSON-LD removed when nav to page without JSON-LD", async ({ page }) => {
		...
		await loadPage(page, "/head-full")
		expect(
			await page.locator("head script[type='application/ld+json']").count(),
...
test.describe("Favicon resolution", () => {
	test("all favicon types rendered on SSR", async ({ page }) => {
		...
		await expect(page.locator("head link[rel='icon'][sizes='any']")).toHaveAttribute(
			"href",
			"/favicon.ico",
		)
...
test.describe("No duplicate tags across rapid navigations", () => {
	test("rapid 4-hop: head-full → head-demo → head-full → head-demo — no duplicates", async ({
		page,
	}) => {
		const cap = setupConsoleCapture(page)
		await loadPage(page, "/head-full")
...
test.describe("3-level head hierarchy SPA transitions", () => {
	test("head-3-level → home → head-3-level round trip preserves 3-level config", async ({
		page,
	}) => {
		const cap = setupConsoleCapture(page)
		await loadPage(page, "/head-3-level/head-page")

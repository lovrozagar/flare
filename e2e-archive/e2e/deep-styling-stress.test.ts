...
test.describe("Stress: mixed styles() + css= + class= Tailwind", () => {
	...
	test.skip("styles() and css= elements have data-c; class= Tailwind element uses class attr", async ({ page }) => {
		await loadPage(page, "/styling-stress")

		/* styles() and css= use data-c */
		for (const id of ["stress-mixed-scoped", "stress-css-native"]) {
			const val = await page.getByTestId(id).getAttribute("data-c")
			expect(val).toBeTruthy()
		}

		/* class= Tailwind element has no data-c — uses class attr */

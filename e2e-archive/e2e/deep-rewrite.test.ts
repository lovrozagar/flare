...
test.describe("Rewrite — Link href output", () => {
	test("Link to=/rewrite-target generates href=/alt-target via output rewrite", async ({
		page,
	}) => {
		await loadPage(page, "/alt-target")
...
test.describe("Rewrite — not-found handling", () => {
	...
	test.skip("SPA: navigate to non-rewritten unknown path shows not-found", async ({ page }) => {
		await loadPage(page, "/")
		await setNavMarker(page)
